...
# day069 生成与 Prompt（M6-D8）：/retrieval/generate、/retrieval/prompt/versions
# 与 /retrieval/grounding/verify 消费的公开接口。
#
# 这一行"按模块名导入"是上面那条纪律的第三次应用，理由逐字相同：
# 引用核对（``_grounding_report``）**只有一份实现**——生成链路、
# /retrieval/answer 与这里的 /retrieval/grounding/verify 用的是同一个函数。
# 在端点里再写一遍（解析 ``[n]`` + 按编号表分三类）不会报错，只会在某一天让
# 两个结论分家（端点说"没有幻觉引用"、生成器说"有一条"），
# 而那种分家在报告里看不出来——它只表现为一次"结论对不上"的争论。
from smart_research_agent.retrieval import generation as generation_module

# day068 重排（M6-D7）：/retrieval/rerank 那三个端点消费的公开接口。
#
# 这里多一行"按模块名导入"（与 ``hybrid`` 的做法逐字相同，理由也逐字相同）：
# 把重排结果写回 ``RetrievalHit`` 的那段逻辑（``_rebuild_hits``）**只有一份实现**
# ——单路第 6.5 步、混合第 10.5 步与这里的 /retrieval/rerank 用的是同一个函数。
# 在端点里再写一遍（replace 三个字段）不会报错，只会在某一天让报告少一列
# （例如漏了 ``channels`` 或 ``stage1_rank``），而那种缺失没有任何报错指向它。
from smart_research_agent.retrieval import retriever as retriever_module

# day071 RAG 评估（M6-D9）：/rag/eval/* 两个端点消费的公开接口。
# 这一组**不重写任何评估逻辑**：指标、归因、基线与对比全在 ``rag_debug`` 包里，
# 端点只做"参数 → 调套件 → 序列化"。理由与上面那几组"按模块名导入"逐字相同：
# 在端点里再算一遍召回率不会报错，只会在某一天让脚本与端点给出两份不同的报告。
#
# ``build_suite`` 必须**改名导入**：本文件顶部已经从 ``finetune_eval`` 导入了
# 同名的 ``build_suite``（微调评估套件的装配器，见那两处调用点）。直接再导入一次
# 会把那个名字**重新绑定**到本层的装配器上——于是微调评估的两个端点
# 会静默地拿到一份 RAG 评估套件，而它们唯一的症状是"返回的字段都不认识"。
# 名字冲突在 Python 里不报错，所以这里靠别名把两个世界分开。
...
def rag_ops_pipeline(request: Request) -> RagOpsPipeline:
    """从 ``app.state`` 取 RAG 运维流水线（day072 本组端点的公共依赖）.

    与 ``indexing_pipeline`` 同一取向：**取不到就给 503**，而不是现装一条。

    现装一条看起来更"友好"，但它会让"我这个 app 体检的是哪个库"变成一个
    需要读装配代码才能回答的问题——而本组端点唯一不能妥协的正是
    "体检对象 = 服务对象"。503 的消息里直接写出注入口
    （``create_app(rag_ops_pipeline=...)`` 或 ``create_app(rag_ops_dir=...)``），
    因此它是一个可以照做的错误，而不是一个"等等看"的错误。
    """
    pipeline = getattr(request.app.state, "rag_ops_pipeline", None)
    if pipeline is None:
        raise HTTPException(
            status_code=503,
            detail="本实例没有装配 RAG 运维流水线：create_app 未注入 rag_ops_pipeline，"
            "且默认装配也没有产出（见 create_app 的 rag_ops_dir 说明）。",
        )
    return pipeline
...
def retrieval_defaults() -> dict[str, Any]:
    ...
    day068 再补六项（``retrieval_rerank_*``）：理由与 day067 那六项逐字相同
    （同样"只改变怎么问"、同样"默认不生效"），而且它们与 ``/retrieval/rerank/status``
    的 ``defaults`` 是**同一份口径**（后者只多了 ``modes`` 与 ``features`` 两张清单）
    ——两份手写的缺省表一定会分家，那时"端点回显的参数"与"真正生效的参数"
    会不一致，而那种不一致在报告里看不出任何异常。

    day069 再补四项（``retrieval_prompt_version`` / ``retrieval_answer_temperature`` /
    ``retrieval_answer_max_tokens`` / ``retrieval_require_citation``）：它们与上面两组
    是同一类（都"只改变怎么问"、都不改任何产物的字节），但差别必须写在这里——
    前两组改的是**名单**（次序、条数），而这四项会**改交付给用户的那段答案文本**：
    同一份索引、同一个问题，换一版提示词交给用户的是两段不同的话。
    因此它们的当前取值更该被读出来（报告按 ``prompt_version`` 分组，
    "换一版之后可核对率变了吗"要靠它可复现）。
    """
    return {
        ...
        "rerank_mode": settings.retrieval_rerank_mode,
        "rerank_weight": settings.retrieval_rerank_weight,
        "rerank_min_score": settings.retrieval_rerank_min_score,
        "prompt_version": settings.retrieval_prompt_version,
        "answer_temperature": settings.retrieval_answer_temperature,
        "answer_max_tokens": settings.retrieval_answer_max_tokens,
        "require_citation": settings.retrieval_require_citation,
    }
...
def retrieval_answer(req: RetrievalAnswerRequest, request: Request) -> RetrievalAnswerResponse:
    ...
    **做引用核对（day069 的 ``GroundingReport``）**，但**不做答案质量评估**
    （有用性 / 准确性打分属于 day071）。
    """
    retrieval_router_ = retrieval_router(request)
    try:
    ...
    return RetrievalAnswerResponse(
        ...
        route=decision.to_dict(),
        # day069 新增的两个键（键在 RetrievalAnswerResponse 里，值来自同一份
        # RagAnswer）：空检索那条路上 check=None、reason=no_context——
        # "没有核对可言"与"核对没通过"是两件事，前者不该被渲染成后者的样子。
        check=answer.check.to_dict() if answer.check is not None else None,
...
# --------------------------------------------------------------------------- #
# day068 重排序（M6-D7）：/retrieval/rerank、/retrieval/rerank/status
#                     与 /retrieval/rerank/compare
#
# 这三条与上面七条**共用同一批注入对象**（``app.state.vector_store`` /
# ``app.state.embedding`` / ``app.state.retrieval_router``），因此问的是
# 同一个库的第七、第八、第九类问题：
#
# ```text
# 这句话取回的那几条，换个更准的排序会变成什么样？  → /retrieval/rerank
# 这台机器上的重排被装配成什么样、用的是哪个模型？  → /retrieval/rerank/status
# 换排序到底有没有用（三条指标的前后对照）？        → /retrieval/rerank/compare
# ```
#
# 四条纪律，每一条都对应本段里的一个具体写法：
#
# 1. **导入期不构造、不读盘、不联网**（与上面七条同源）：本段只有常量、函数与
#    路由装饰器；重排器与路由器全在**请求期**取或现装（见 ``retrieval_reranker``）。
# 2. **本组有一个"强制开启"的口子**：重排在检索链路上**默认关着**
#    （``retrieval_rerank_enabled=False``，理由见 config：它改变的是名单次序，
#    因此"换一层排序"必须是一次显式决定）。而 ``/retrieval/rerank`` 的存在理由
#    就是"让人看见次序被改动了"，因此这条通道**强制开启**：请求体里
#    ``enabled=False`` 是一个明确被拒的请求（400），而不是"照做但什么都不做"。
# 3. **端点不重算任何检索与重排决策**：窗口、模式、归一化、排序键、三条指标、
#    诊断全部留在 ``retrieval`` 包里（那里有它们的唯一实现）。端点只做三件事：
#    折参数、调检索层、把结果序列化。写回命中用的是**同一个**原语
#    （``rerank_hits`` 与 ``retriever._rebuild_hits``，与单路第 6.5 步、
#    混合第 10.5 步是同一份实现）——两份各写一遍的写回一定会分家，
#    而分家不会报错，只会让报告少一列。
# 4. **"前后两个次序"必须同时出现在响应里**：只给重排后的名单，
#    读的人无法判断"重排到底动了什么"（"一条都没动"与"名单本来就是这样"
#    看起来完全一样）。
#
# 错误码与上面七条同一套语言：**请求参数问题一律 400**（重排的四族参数
# top_n / mode / weight / min_score 与检索那一组共用同一条通道），
# **装配问题（注入对象类型不对）一律 500**（见 ``retrieval_reranker``），
# 认不出来的异常上抛成 500——报告只收留我们认识的失败。
# --------------------------------------------------------------------------- #

#: 重排器挂在 ``app.state`` 上的键名（可选）。**缺省不需要它**：
#: 本段会用 ``settings.retrieval_rerank_model`` 现装一个教学级替身
#: （``CrossEncoderReranker``），于是"缺省 app 的 /retrieval/rerank/* 就能用"
#: 这件事不需要 ``create_app`` 认识重排这一层。要接真实重排模型请注入一个
#: ``BaseReranker`` 子类——这是本课给生产环境留的唯一接口
#: （本层**不按名字加载模型**，``model`` 参数只进报告）。
RETRIEVAL_RERANK_STATE_KEY = "retrieval_rerank"

#: 状态端点的 ``note``：**"教学级替身"的诚实声明**（写在最前面，不藏在细节里）。
#:
#: 它与另两处同源、三处都写是刻意的：``CrossEncoderReranker.summary_line()``
#: （人读的一行）、``describe()["kind"]`` 里的 ``(teaching)``（机器读的标记）
#: 与这里的 ``note``（端点响应体里必须看得见的那句话）。
#: 只截其中一处去做结论的人，会在另外两处撞见同一句声明。
RERANK_TEACHING_NOTE = (
    "这是**教学级替身**（cross-encoder-teaching-v1）：分数由四个可手算特征"
    "加权而成，**它没有真实语义**（认不出同义改写、读不懂语序），"
    "因此分数只能用于**相对排序**，不能当成'相关性'读；"
    "生产环境请替换为重排模型——接口是 rerank.BaseReranker，"
    "换实现时不需要改本项以外的任何配置（本层不按名字加载模型）。"
)


def retrieval_reranker(request: Request) -> BaseReranker:
    """取重排器：**注入优先**，否则按 ``settings.retrieval_rerank_model`` 现装一个替身.

    为什么每次请求现装而不是缓存：它只持有一个名字与三个计数器（构造是 O(1)
    且不读任何东西），而缓存会让"注入的重排器被换掉"这件事在某个请求上悄悄失效
    ——与 ``retrieval_router`` / ``retrieval_lexical`` 是同一条纪律。

    代价必须写下来：**缺省装配下 ``describe()`` 的三个计数器永远是 0**
    （每次请求都是一个新实例）。要看累计账（"这台机器一共打了多少次分"）
    请注入一个实例挂在 ``app.state`` 上——那时它们才是真正的累计值。
    另一种看法是：缺省装配下这三个数**如实**报告"这个实例还没打过分"，
    而不是报一个看起来像全局统计的数。

    注入了一个非 ``BaseReranker`` 的对象时给 500 而不是 400：那是装配写错了，
    不是请求参数写错了——把它降级成 400 会让一个必然复现的配置错误
    看起来像"这次请求碰巧不对"。
    """
    injected = getattr(request.app.state, RETRIEVAL_RERANK_STATE_KEY, None)
    if injected is not None:
        if not isinstance(injected, BaseReranker):
            raise HTTPException(
                status_code=500,
                detail=f"app.state.{RETRIEVAL_RERANK_STATE_KEY} 必须是 BaseReranker，"
                f"收到 {type(injected).__name__}：这是装配问题而不是请求参数问题，"
                "把它当成 400 会让一个必然复现的配置错误看起来像一次偶然。"
                "出路：写一个 BaseReranker 子类（实现 score_pairs 与 name）再注入，"
                "不要传一个函数或一个字符串模型名——本层**不按名字加载模型**。",
            )
        return injected
    return CrossEncoderReranker(name=settings.retrieval_rerank_model)


def rerank_defaults() -> dict[str, Any]:
    """``config`` 里 ``retrieval_rerank_*`` 那一组的当前取值（"为什么是这个数"的出处）.

    与 ``retrieval_defaults`` 是同一套写法（能力与当前取值必须分开），只是换成
    重排这一组。``modes`` 与 ``features`` 也列进来：前者是 ``mode`` 的**合法取值
    清单**（"写错了 mode"这条错误里必须能抄到可用值），后者是教学替身摊开来的
    四个特征名（"这一条为什么得到这个分"要按一张固定的表读）。
    """
    return {
        "enabled": settings.retrieval_rerank_enabled,
        "model": settings.retrieval_rerank_model,
        "top_n": settings.retrieval_rerank_top_n,
        "mode": settings.retrieval_rerank_mode,
        "weight": settings.retrieval_rerank_weight,
        "min_score": settings.retrieval_rerank_min_score,
        "modes": list(RERANK_MODES),
        "features": list(RERANK_FEATURE_NAMES),
    }


def resolved_rerank_params(
    *,
    top_n: int | None,
    mode: str | None,
    weight: float | None,
    ...
    """请求体里的重排参数 → ``rerank_hits`` 的参数（``None`` 在这里换成 settings 的当前值）.

    为什么要有这一步而不是直接把 ``None`` 递下去：``rerank_hits`` 的 ``None``
    意思是**检索层缺省**（``DEFAULT_RERANK_*``），而请求体文档里承诺的是
    **这台机器的配置**（``settings.retrieval_rerank_*``）。两者今天恰好相等
    （20 / replace / 0.5），但配置一旦被改，只递 ``None`` 的写法会让端点
    悄悄忽略配置——而"端点回显的参数"与"真正生效的参数"不一致，
    是这一层最贵的一类错误（报告里那个 top_n 会是编出来的）。

    取值本身的**合法性**不在这里判：``rerank_hits`` 是唯一一份实现
    （``top_n >= 1``、``mode`` 落在封闭清单、``weight`` 落在 ``[0, 1]``），
    它给出的消息里带着合法取值。端点上再判一遍的后果是两份规则迟早分家。
    """
    return {
        "top_n": settings.retrieval_rerank_top_n if top_n is None else top_n,
        "mode": settings.retrieval_rerank_mode if mode is None else mode,
        "weight": settings.retrieval_rerank_weight if weight is None else weight,
        "min_score": (
            settings.retrieval_rerank_min_score if min_score is None else min_score
        ),
    }


def reranked_result_of(result: RetrievalResult, reranked: RerankResult) -> RetrievalResult:
    """把重排结果写回一份 ``RetrievalResult``（**复用检索层唯一的写回实现**）.

    三件事按顺序做，与 ``Retriever._apply_rerank`` 的第 6.5 步是同一件事：

    ```text
    1. 写回命中   retriever._rebuild_hits：新名次 + rerank_score + stage1_rank
                  （text / metadata / channel / channels / score 全部取自原命中）
    2. 记两笔账   rerank 摘要（RerankResult.to_summary）与 dropped_by_rerank
    3. 补诊断     empty_reason 只在"重排阈值把命中切光"时才改写（见下）
    ```
    为什么用 ``dataclasses.replace`` 而不是手工拼一个字典：拼出来的那份
    **不再受任何不变量检查**（在库的那份里，``hits`` 与 ``empty_reason``
    互相矛盾会当场报错）。这里让它仍是一份真的 ``RetrievalResult``，
    于是"响应体里那一块"与"脚本里那一份"是同一个形状。

    第 3 步的三条分支各自对应一种事实：

    ```text
    还有命中                      → "hits"（有命中时唯一合法的取值）
    上游本来就空（候选 0）        → 保留上游的原因（no_data / filtered_out）：
                                   重排那条早退路径一次都没调用重排器
    有候选却一条不剩              → "below_threshold"：只可能是重排阈值切光的，
                                   取值与检索层 _diagnose 的第四个数保持一致
    ```
    """
    hits = retriever_module._rebuild_hits(list(result.hits), reranked)
    if hits:
        empty_reason = EMPTY_REASON_NONE
    elif result.count == 0:
        empty_reason = result.empty_reason
    else:
    ...
    return replace(
        result,
        hits=tuple(hits),
        rerank=reranked.to_summary(),
        dropped_by_rerank=reranked.dropped_by_min_score,
        empty_reason=empty_reason,
        notes=tuple(result.notes) + tuple(f"重排：{note}" for note in reranked.notes),
    )


@router.post("/retrieval/rerank", response_model=RerankResponse)
def retrieval_rerank(req: RerankRequest, request: Request) -> RerankResponse:
    """一句话 → 检索 → **强制重排** → 重排前后的完整交代（day068）.

    它与 ``/retrieval/search`` 的关系值得写清楚：**同一个库、同一句话、
    同一批候选、同一个 ``RetrievalResult`` 形状**，差别只在最后那一步
    ——"排序规则换成了交叉编码器重排"。因此它不重复解释那五个数字
    （``fetch_k`` / ``candidates`` / 三个 ``dropped_*``），只回答一件事：

    ```text
    before_ids   第一阶段给的名单（谁来当第一名由它决定）
    after_ids    重排之后的名单（本端点存在的理由：让人看见次序被改动了）
    rerank       窗口的账（candidates / scored / window / top_n）、模式与权重、
                 逐条的四个特征、以及**注记**（窗口纪律那条是无条件的）
    result       写回之后的 RetrievalResult：命中带 rerank_score 与 stage1_rank，
                 而 score 仍然是第一阶段的分数（口径不变，两个分数各占一个字段）
    ```

    四条边界行为，各自对应一个不同的动作：

    ```text
    enabled=False（明确要求关掉重排）→ 400，消息说明"这条通道强制开启"与关掉它的位置
    空库 / 过滤后无命中             → 200，重排**一次都不调用重排器**（计数器不动），
                                      empty_reason 保留上游那一个（no_data / filtered_out）
    top_n 小于候选数                → 200，窗口外的命中按原顺序接在后面（一条都没打分），
                                      "窗口外 N 条"那条注记进 lines 与 rerank.notes
    min_score 把窗口内切光          → 200 且 empty_reason="below_threshold"，切掉几条
                                      由第四个 dropped_* 数（dropped_by_rerank）记账
    ```

    参数非法一律 **400**，消息里带合法取值（``top_n`` 下界 / ``mode`` 的封闭清单 /
    ``weight`` 的区间 / 可用路由名都由检索层给出）。重排那六个参数全部可省略，
    ``None`` 一律取 ``settings.retrieval_rerank_*``（见字段说明）。

    **``min_score`` 在这里是重排阈值**（比的是重排分，不是第一阶段的分数）：
    两者同名不同物，因此请求体里这一个数字在折成检索查询时被**显式清空**
    （``None`` = 让检索层用它自己的缺省 ``settings.retrieval_min_score``），
    只落一次刀——落在重排那一侧。不这么写的话同一个数字会落两次刀，
    而两次比的分数根本不是同一种量纲（第一阶段无上界、重排分落在 ``[0, 1]``），
    于是 ``min_score=0.5`` 会先把召回切到只剩一条、再在重排那一侧把它切掉。
    要设第一阶段的阈值请用 ``/retrieval/search``。

    它**不做什么**：不写库、不重建索引、不改任何文档的向量（重排只改**次序**，
    因此它不会让任何文件变一个字节）；**只对前 N 条（``top_n``）打分**，
    窗口之外的命中一条都不打分、名次只由第一阶段决定（窗口纪律，
    见 ``RETRIEVAL_LIMITATIONS`` 第一条）；用的重排器是**教学级交叉编码器替身**，
    它没有真实语义、分数只能作相对排序用（同上第二条）；也不做多路融合（day067）、
    不做重排模型的训练与微调（那在 out_of_scope 里）、不替调用方解释
    "这条为什么没被召回"（那是评估层的事）。
    """
    if req.enabled is False:
        # **拒绝而不是忽略**：静默忽略这个参数会让"我明明关了重排却看到次序变了"
        # 变成一个查不出病因的现象（与 /retrieval/hybrid 不静默退回单路同一条纪律）。
        raise HTTPException(
            status_code=400,
            detail="本端点**强制开启**重排，不接受 enabled=False：它存在的理由就是"
            "'让人看见次序被改动了'——关掉重排之后，它只剩下 /retrieval/search "
            "已经给过的那一份名单。出路：省略 enabled（或写 true）；要关掉重排请改用 "
            "/retrieval/search——那个开关（settings.retrieval_rerank_enabled，"
            "环境变量 RETRIEVAL_RERANK_ENABLED）管的是**检索链路上的缺省装配**。",
        )
    reranker = retrieval_reranker(request)
    retrieval_router_ = retrieval_router(request)
    try:
        # **把 min_score 从检索查询里清掉**（本段最容易写错的一处）：
        # 请求体里的 min_score 是**重排阈值**，而 ``retrieval_query_of`` 会把它
        # 原样折进 ``RetrievalQuery.min_score``——也就是**第一阶段的阈值**。
        # 不清掉的话，同一个数字会落两次刀：``min_score=0.5`` 先把召回切到
        # 只剩分数 >= 0.5 的那几条，再在重排那一侧按重排分切一次，
        # 而两次落刀的"分数"根本不是同一种量纲（第一阶段无上界、重排分落在 [0, 1]）。
        # ``None`` 在这里的含义是"让检索层用它自己的缺省"（settings.retrieval_min_score），
        # 与请求体没写 top_k 时的处理是同一句话。要设第一阶段的阈值请用 /retrieval/search。
        query = replace(retrieval_query_of(req), min_score=None)
        _retriever, result, decision = retrieval_run(retrieval_router_, query)
        reranked = rerank_hits(
            result.hits,
            # 查询文本取**规范化过的那一份**（``result.query``）而不是请求体里的原串：
            # 检索真正编码的是它（strip 之后），而重排分是对 (查询, 文档) 打的——
            # 两边各用一句"看起来一样"的文本，会让重排的分数与检索的名单
            # 悄悄对不上（差别只有首尾空白时，这种事不会报错）。
            result.query.text,
            reranker=reranker,
            model=req.model,
            **resolved_rerank_params(
                top_n=req.top_n,
                mode=req.mode,
                weight=req.weight,
                min_score=req.min_score,
        ...
        # 参数问题（QueryError / RerankError）与索引问题（IndexStateError）都继承自
        # RetrievalError，消息里已经带足出路，原样带回即可；
        # 认不出来的异常一律上抛成 500（报告只收留我们认识的失败）。
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    final = reranked_result_of(result, reranked)
    return RerankResponse(
        summary=final.summary_line(),
        result=final.to_dict(include_text=True),
        rerank=reranked.to_dict(),
        before_ids=result.ids(),
        after_ids=[hit.record_id for hit in reranked.hits],
        lines=reranked.explain(),
        route=decision.to_dict(),
        empty_reason=final.empty_reason,
        empty_reason_description=EMPTY_REASON_DESCRIPTIONS.get(final.empty_reason, ""),
        defaults=rerank_defaults(),
        limitations=list(RETRIEVAL_LIMITATIONS),
        out_of_scope=list(RETRIEVAL_OUT_OF_SCOPE),
    )


@router.get("/retrieval/rerank/status", response_model=RerankStatusResponse)
def retrieval_rerank_status(request: Request) -> RerankStatusResponse:
    """重排的现状（day068）：模型自述 / 开关 / 四个参数 / 缺省值表.

    它存在的理由是**诊断顺序**。读到一份"重排好像没起作用"的结果时，
    要按这个顺序排除：

    ```text
    1. 用的是哪个重排器、它的四个特征权重是多少  → model（含 (teaching) 标记）
    2. 检索链路上到底开没开                     → enabled（/retrieval/search 要不要重排）
    3. 窗口多大、哪种模式、权重多少、有没有阈值   → top_n / mode / weight / min_score
    4. 这些数是项目默认还是谁改过               → defaults（与上一行刻意重复，见下）
    ```

    第 3 行与第 4 行**刻意重复**（``top_n`` 与 ``defaults["top_n"]`` 是同一个数）：
    调用方最常问的一句是"这次生效的是哪一个值"，让它出现在顶层就不必解析嵌套字典；
    而 ``defaults`` 里那一份回答的是"这个数是项目默认还是被改过"
    ——与 ``/retrieval/status`` 的 ``index`` 那一段同一条理由。

    ``enabled=False`` 时本端点**照样返回 200**：它回答的是"现在是什么状态"，
    而不是"能不能重排"。``/retrieval/rerank`` 是一条**强制开启**的通道，
    因此"链路上关着"与"我能不能看重排"是两件事——把状态端点也关掉，
    会让"为什么 /retrieval/search 的结果没有 rerank 字段"无从查起。

    ``note`` 里是**教学级替身的诚实声明**（一句话，见 ``RERANK_TEACHING_NOTE``）：
    响应体里必须看得出"这次打分的是替身而不是真实模型"。

    它**不做什么**：不触发任何检索、不重排任何东西、不写任何状态
    （纯粹的读端点）；**不报告"重排有没有用"**——那要靠金标准与三条指标，
    因此它是另一个端点（``/retrieval/rerank/compare``）；也**不代表生产配置**：
    这里端出来的是这台机器上那一个替身与它的缺省参数。
    """
    reranker = retrieval_reranker(request)
    return RerankStatusResponse(
        summary=reranker.summary_line(),
        model=reranker.describe(),
        enabled=settings.retrieval_rerank_enabled,
        top_n=settings.retrieval_rerank_top_n,
        mode=settings.retrieval_rerank_mode,
        weight=settings.retrieval_rerank_weight,
        min_score=settings.retrieval_rerank_min_score,
        note=RERANK_TEACHING_NOTE,
        defaults=rerank_defaults(),
        limitations=list(RETRIEVAL_LIMITATIONS),
        out_of_scope=list(RETRIEVAL_OUT_OF_SCOPE),
    )
...
@router.post("/retrieval/rerank/compare", response_model=RerankCompareResponse)
def retrieval_rerank_compare(
    req: RerankCompareRequest, request: Request
) -> RerankCompareResponse:
    """一句话 + 金标准 id → 三条指标的前后对照（day068）：**重排有没有用**.

    它回答的是"重排这件事值不值"，而这个问题不能靠感觉回答，也不能靠
    "重排前后两条看起来不一样"回答（那只说明次序变了，不说明变好了）。
    服务端自己做四件事，顺序固定：

    ```text
    1. 检索一次         retrieve(probe.query) —— 本端点只检索**一次**
    2. 算 before        result.ids() 对三条指标（recall@k / RR / nDCG@k）
    3. 重排同一批命中   rerank_hits(result.hits, …) —— 因此 before/after 只差一个变量：
                        "有没有重排"，而不是"检索了两次"
    4. 算 after 与 lift 逐条明细进 per_query（**长度恒为 1**：输入只有一条查询）
    ```

    第 3 步"用同一批命中"很要紧：如果 after 是"重排之后重新检索一遍"，
    那么两张指标表的差里就混着两次检索的随机性——而这份报告的全部价值
    就在于**把变量减到一个**。

    三条指标必须一起看（公式见 ``rerank`` 模块 docstring）：``recall`` 不看排名，
    ``RR`` 只看第一条相关，``nDCG`` 是唯一同时看"命中几条"与"排得多靠前"的。
    ``lift`` 用**差值**而不是比值：``before == 0`` 时比值是无穷。

    四条边界行为：

    ```text
    空金标准（relevant=[]）   → 400，消息点明"空标注会让 recall@k 变成 0/0"
    k 越界（0 / 负数）        → 400，消息点明"k=0 会让两条指标恒为 0"
    未知 mode                → 400，消息里列出可用模式（replace / blend）
    空库                     → 200 且三条指标都是 0（合法状态，不是 500）——
                               "评了 1 条查询但一条都没召回到"必须能被看见
    ```

    参数非法一律 **400**（同一套错误码语言）。``top_n`` / ``mode`` / ``weight``
    全部可省略，``None`` 取 ``settings.retrieval_rerank_*``。

    它**不做什么**：不写库、不落盘、**不设重排阈值**（那改变的是名单长度，
    不是排序质量，混进对照表会让"提升"同时指两件事）；不选路
    （测的是这台机器的缺省检索路径，多索引又没设默认路由时会 400 并列出可用名）；
    **也不代表真实重排模型的收益**——这里量的是教学级替身在同一份语料上的
    前后差（替身也能跑出一份对照表，因为三条指标只依赖"名单 + 金标准"，
    与用哪个模型无关）；重排模型的训练与微调不在本课内（见 out_of_scope）。
    """
    retrieval_router_ = retrieval_router(request)
    reranker = retrieval_reranker(request)
    params = resolved_rerank_params(top_n=req.top_n, mode=req.mode, weight=req.weight)
    try:
        report = measure_lift(
            [LiftProbe(query=req.query, relevant=tuple(req.relevant))],
            # 探针 → 一份 RetrievalResult：走路由器（与上面七条同一套选路规则），
            # 因此"接口量的那次检索"就是"脚本量的那次检索"。
            lambda text: retrieval_router_.retrieve(RetrievalQuery(text=text)),
            reranker=reranker,
            k=req.k,
            top_n=params["top_n"],
            mode=params["mode"],
    ...
    return RerankCompareResponse(
        summary=report.summary_line(),
        before=dict(report.before),
        after=dict(report.after),
        lift=dict(report.lift),
        per_query=[dict(item) for item in report.per_query],
...
# --------------------------------------------------------------------------- #
# day069 生成与 Prompt（M6-D8）：/retrieval/generate、/retrieval/prompt/versions
#                     与 /retrieval/grounding/verify
#
# 这三条与上面十条**共用同一批注入对象**（``app.state.vector_store`` /
# ``app.state.embedding`` / ``app.state.retrieval_router`` /
# ``app.state.retrieval_llm``），因此问的是同一个库的第十、第十一、第十二类问题：
#
# ```text
# 这句话取回的那几条，会被问成什么、答成什么、答案能不能被核对？ → /retrieval/generate
# 现在生效的是哪一版提示词、那一版改了什么、六种回退分别是什么？ → /retrieval/prompt/versions
# 就这段答案与这份编号表：有没有引用了不存在的编号？            → /retrieval/grounding/verify
# ```
#
# 四条纪律，每一条都对应本段里的一个具体写法：
#
# 1. **导入期不构造、不读盘、不联网**（与上面十条同源）：本段只有常量、函数与
#    路由装饰器；生成器在**请求期**按请求体现装（见 ``retrieval_generate``）。
# 2. **LLM 的判据只有一份**（``retrieval_llm``）：它已经实现了"未配置 / 仍是
#    MockLLM 占位 → 400"，本段不另写一份——两份判据一旦分家，同一个 app 会在
#    ``/retrieval/answer`` 上被拦、在 ``/retrieval/generate`` 上被放过。
# 3. **端点不重算任何核对**：三个编号集合、覆盖率、接地、逐编号的检查行全部来自
#    ``generation``（那里有唯一的实现）。本段唯一的"计算"是**只读元信息**
#    （模板字符数 / 占位符 / 约束条数），它不进任何判定。
# 4. **核对可以单独问一次**（``/retrieval/grounding/verify``）：它一次模型都不调，
#    因此"有没有幻觉引用"能在不花钱、不联网、不需要可用模型的前提下被复核。
#    这也是本组唯一一条**不读注入对象**的写操作——它只读请求体。
#
# 错误码与上面十条同一套语言：**请求参数问题一律 400**（检索参数、打包预算与
# 生成参数四族共用同一条通道），**装配问题（注入对象类型不对）一律 500**，
# 认不出来的异常上抛成 500——报告只收留我们认识的失败。
# --------------------------------------------------------------------------- #

#: 提示词模板里"约束"小节的标题（v1 与 v2 各有一个，见 ``PROMPT_CHANGELOG``）。
#: 只服务端点那份**只读元信息**（"这一版写了几条约束"），不参与任何校验：
#: 模板的校验（占位符齐不齐、格式串合不合法）只在 ``generation`` 的构造期做一次。
_PROMPT_CONSTRAINT_HEADINGS: tuple[str, ...] = ("回答要求：", "## 约束")

#: 模板里的编号条目（"1. " / "12. "）：数"这一版写了几条约束"用。
_NUMBERED_ITEM = re.compile(r"^\s*\d+[.、]\s")


def prompt_placeholders(template: str) -> list[str]:
    """模板里的占位符名字（升序去重）——**只读呈现**，不是校验.

    它读的是**格式串的语法**（``string.Formatter().parse``，与 ``generation``
    构造期校验用的是同一个原语）：``{{context}}`` 这种转义写法因此不会被算成
    一个占位符——"在字符串里搜 ``{context}``"的写法会把它算进去，而那种模板
    渲染出来是一段谁也读不懂的文本。

    校验**不在这里**：缺占位符是 ``ContextError``，那句话与那份判据都住在
    ``generation._validate_prompt``。端点上再判一遍的后果是两份规则迟早分家，
    而分家的表现是"某个模板在这里被点名、在真正渲染时又通过了"。
    """
    return sorted(
        {
            field_name
...
def prompt_constraints(template: str) -> int:
    """这一版模板写了几条**编号约束**（数到下一个二级标题为止）.

    为什么要数它：``RAG_PROMPT_V2`` 把约束从 4 条扩到 6 条，而且**每一条都注明了
    它挡哪一种真实失败**。"这一版写了几条"因此是换版本时最该被看见的一个数
    ——文案改了而条数没加时，读的人会以为那六种失败都被挡过了。

    口径挂在**小节标题**上（v1 是 ``回答要求：``、v2 是 ``## 约束``）而不是
    "数一数全文所有的 ``1. ``"：v2 的 ``## 输出格式`` 一节也是编号列表，
    把它算进来会让这个数从 6 变成 9——而 9 不回答"v2 比 v1 多挡了哪两种失败"。
    """
    ...
    start = next(
        (
            ...
            if any(heading in line for heading in _PROMPT_CONSTRAINT_HEADINGS)
        ),
        None,
    )
    if start is None:
...
def prompt_templates() -> list[dict[str, Any]]:
    """受管模板的逐版元信息（``/retrieval/prompt/versions`` 直接返回它）.

    五栏各自回答一个不同的问题：

    ```text
    version        这一版叫什么（报告与评估的分组键）
    chars          它有多少字（提示词预算的参照：模型读到的每一句都在这段里）
    changelog      为什么有这一版（来自 generation.PROMPT_CHANGELOG，不是端点现编）
    placeholders   它要求哪些占位符（缺一个就是一次"没有资料的提问"）
    constraints    它写了几条编号约束（每一条都该注明挡哪一种真实失败）
    ```

    ``changelog`` 直接读 ``PROMPT_CHANGELOG``：那份注释与模板同住一处
    （"模板住哪，清单就住在哪"，见 ``generation`` 的模块 docstring），
    端点上再写一遍必然与它分家——而分家之后，"这一版改了什么"会有两个答案。
    """
    return [
        {
            "version": version,
            "chars": len(RAG_PROMPTS[version]),
            "changelog": PROMPT_CHANGELOG.get(version, ""),
            "placeholders": prompt_placeholders(RAG_PROMPTS[version]),
            "constraints": prompt_constraints(RAG_PROMPTS[version]),
        }
...
def generation_defaults() -> dict[str, Any]:
    """``config`` 里生成那一组的当前取值 + 两张**封闭清单**（"为什么是这个数"的出处）.

    与 ``rerank_defaults`` 是同一套写法（能力与当前取值必须分开），只是换成生成
    这一组；两张清单也列进来，因为它们都是"写错了要在消息里抄得到"的那类东西：

    ```text
    prompt_versions   受管版本号（prompt_version 的合法取值）
    decline_markers   固定拒答句式（检测清单 = v2 的约束 2，同一份）
    ```

    为什么不像 ``rerank_defaults`` 那样把回退原因也塞进来：那是**六条人话**，
    本组已经给它们留了一个专门的字段（``/retrieval/prompt/versions`` 的
    ``fallback_reasons``）——塞进缺省值表只会让那张表变成一张长表，
    而"这个数是项目默认还是谁改过"这个问题的答案会被淹掉。
    """
    return {
        "prompt_version": settings.retrieval_prompt_version,
        "answer_temperature": settings.retrieval_answer_temperature,
        ...
        "decline_markers": list(DECLINE_MARKERS),
    }


@router.post("/retrieval/generate", response_model=GenerateResponse)
def retrieval_generate(req: GenerateRequest, request: Request) -> GenerateResponse:
    """一句话 → 检索 → 打包 → **生成** → 一份可核对的账（day069）.

    它与 ``/retrieval/answer`` 的关系值得写清楚：**同一个库、同一句话、同一条护栏**
    （检索 / 打包为空时一次 LLM 都不调），差别只在"生成那一步的交代给多细"。
    ``/retrieval/answer`` 回答"用户会看到什么"；本端点回答"这句话是怎么被问出来的、
    答案里的 ``[n]`` 到底指着谁"，因此它多给三样东西：

    ```text
    prompt_text   真正发出去的那段提示词（"模型这次看到了什么"的唯一证据）
    grounding     三个编号集合 + 覆盖率 + 接地（这份答案能不能被片段核对）
    check         逐编号的检查行（哪一条是幻觉引用、哪几条给了却没被引用）
    ```

    六条边界行为，每一条都对应一个**不同的动作**（与 ``FALLBACK_REASONS`` 同一份清单）：

    ```text
    检索 / 打包为空        → 200 且 llm_called=False、fallback_reason=no_context、
                            citations=[]、answer=兜底答复（**一次 LLM 都不调**）
    调用抛异常             → 200 且 fallback_reason=llm_error（**llm_called=True**：
                            （超时 / 鉴权 / 网络 / 额度）护栏管不了"给了片段之后调用失败"）
    模型返回空串           → 200 且 fallback_reason=empty_reply（answer 就是空串）
    模型自己说资料不足      → 200 且 fallback_reason=model_declined（命中固定句式，
                            注记里写着命中的是哪一句）
    require_citation=True 而无合法引用
                          → 200 且 fallback_reason=unusable_citations（答案改取兜底答复）
    （都没命中）           → 200 且 fallback_reason=""：答案来自模型且核对出了有效引用
    ```

    后四条**都不是错误**：它们是"这次答案不能按正常路径读"的六条话之一，
    因此状态码是 200，结论在 ``fallback_reason`` 与 ``fallback_reason_description`` 里
    ——把可预期的失败抛成异常，调用方就只能拿到一个栈，而拿不到"这次为什么不算数"。

    参数非法一律 **400**，消息里带合法取值：``prompt_version``（受管版本只有
    ``['v1', 'v2']``）/ ``temperature``（``[0, 2]``）/ ``max_tokens``（``>= 1``）
    由生成层给，八个检索参数与两个预算由检索层给（与 ``/retrieval/search`` 同一条通道）。
    **未配置 LLM（或仍是 MockLLM 占位）也是 400**：判据与 ``/retrieval/answer``
    逐字相同（见 ``retrieval_llm``），本段不另写一份。

    它**不做什么**：不写库、不重建索引、不重排序（day068）、不做多路融合（day067）、
    **不改提示词模板**（请求体里没有模板文本这个字段——要换模板请换版本号，
    或在装配期注入一个自带模板的 ``RAGGenerator``）、不做答案质量评估
    （有用性 / 准确性打分属于 day071）。
    """
    retrieval_router_ = retrieval_router(request)
    llm = retrieval_llm(request)
    try:
        # 生成器的装配也放在 try 里：版本号 / temperature / max_tokens 的校验发生在
        # **构造期**，而它们与检索层那些拒绝一样属于"这一次请求的参数不对"，
        # 因此必须一起走 400 那条通道——放在 try 外面会被兜底处理器渲染成 500，
        # 于是一个"传错了版本号"的请求看起来像一次服务器故障。
        generator = RAGGenerator(
            ...
            require_citation=(
                settings.retrieval_require_citation
                if req.require_citation is None
                else req.require_citation
            ),
        )
        query = retrieval_query_of(req)
        _retriever, result, decision = retrieval_run(retrieval_router_, query)
        # 打包在这里做（本端点跑的是一次**完整的 RAG 链路**，不是只管生成那一步）：
        # 预算读 settings，与 RagPipeline 的缺省装配是同一对数——请求体不开放这两个
        # 预算（见 GenerateRequest 的说明），因为它们是"给模型多少空间"的装配决定。
        context = pack_context(
            result.hits,
            max_chars=settings.retrieval_max_context_chars,
            per_hit_chars=settings.retrieval_per_hit_chars,
        )
        generation: Generation = generator.generate(result.query.text, context)
    except RetrievalError as exc:
        # 三族拒绝（检索参数 / 打包配置 / 生成参数）都继承自 RetrievalError
        # ——GenerationError 继承 QueryError，也在这条通道上。消息里已经带足出路，
        # 原样带回即可；认不出来的异常一律上抛成 500（报告只收留我们认识的失败）。
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    return GenerateResponse(
        summary=generation.summary_line(),
        answer=generation.answer,
        llm_called=generation.llm_called,
        prompt_version=generation.prompt_version,
        prompt_text=generation.prompt_text,
        citations=[citation.to_dict() for citation in generation.citations],
        grounding=generation.check.to_dict(),
        fallback_reason=generation.fallback_reason,
        fallback_reason_description=FALLBACK_REASON_DESCRIPTIONS.get(
            generation.fallback_reason, ""
        ),
        notes=list(generation.notes),
        # 检索为空时这里是一份**空包**（count=0 / text="" / citations=[]）而不是 None：
        # 本链路照样走完了"打包 → 生成"，而"这次打包了什么"在空检索时同样可回答。
        context=context.to_dict(),
        retrieval=result.to_dict(include_text=True),
        empty_reason=result.empty_reason,
        route=decision.to_dict(),
        check=[row.to_dict() for row in generation.check.checks],
...
# --------------------------------------------------------------------------- #
# day071 RAG 评估（M6-D9）：/rag/eval/status 与 /rag/eval/run
#
# 这一组问的是**整条链路**的第五类问题——前四个分别属于 day066~day069：
#
# ```text
# 给一句话，取回哪几条？                → /retrieval/search        （day066）
# 两路怎么合、排得对不对？               → /retrieval/hybrid /rerank（day067~068）
# 这句话会被问成什么、答案能不能核对？    → /retrieval/generate      （day069）
# 这一批答案整体好不好、不好在哪一层？    → /rag/eval/run            （day071，本组）
# ```
#
# 四条纪律，每条在本段里都对应一个具体写法：
#
# 1. **导入期不做 I/O**：本段只有常量、辅助函数与路由装饰器；评测集路径、
#    基线文件的存在性检查全在**请求期**（``/rag/eval/status`` 会读一次文件系统，
#    那正是它的职责）。
# 2. **评估逻辑一行都不重写**：指标、归因、基线、对比全在 ``rag_debug`` 包里
#    （与 /retrieval/* 复用检索层是同一条纪律）。端点只做三件事：
#    取注入对象 → 调套件 → 序列化。
# 3. **参数问题一律 400，消息原样带回**：``RagDebugError`` 是这一族的基类，
#    它认识的失败都是**可预期的拒绝**（变体名不存在、评测集缺字段、
#    基线与当前不可比）。认不出来的异常一律上抛 500。
# 4. **未配置 LLM 时也返回 400**（与 /retrieval/answer 同一条通道），
#    但**不排除 MockLLM**——这一条与 day066 的取舍不同，理由必须写出来：
#    /retrieval/answer 的产物是"一段会被当成事实读的答案"，用按脚本回复的
#    占位实现去回答知识库问题正是那一课要拦的失败；而本端点的产物是
#    **一份关于评估的报告**——它如实记着 ``llm_called`` 与六种回退的分布，
#    因此用一个确定性替身跑通整条评测流水线是**离线演示的合法姿势**
#    （``scripts/rag_debug_demo.py`` 就是这么做的；真实评估请注入真模型）。
# --------------------------------------------------------------------------- #


def rag_eval_defaults() -> dict[str, Any]:
    """``config`` 里 ``rag_eval_*`` 那一组的当前取值（"为什么是这个数"的出处）.

    与 ``retrieval_defaults`` 同一个用途，但这一组比它更该被读出来：
    评测的**判据本身就是结论的一部分**——k=3 与 k=5 算出来的召回率不可比，
    而"这批数字是在什么判据下得到的"必须能从响应体里读出来。
    """
    return {
        "cases_path": settings.rag_eval_cases_path,
        "corpus_path": settings.rag_eval_corpus_path,
        "baseline_path": settings.rag_eval_baseline_path,
        "report_path": settings.rag_eval_report_path,
        "top_k": settings.rag_eval_top_k,
        "min_coverage": settings.rag_eval_min_coverage,
        "require_grounded": settings.rag_eval_require_grounded,
        "dead_band": settings.rag_eval_dead_band,
        "min_samples": settings.rag_eval_min_samples,
        "recall_tolerance": settings.rag_eval_recall_tolerance,
        "grounded_tolerance": settings.rag_eval_grounded_tolerance,
        "coverage_tolerance": settings.rag_eval_coverage_tolerance,
        "hallucination_tolerance": settings.rag_eval_hallucination_tolerance,
    }


def rag_eval_baseline_file() -> Path:
    """已提交的质量基线文件路径（**存在性检查在调用处**，这里只拼路径）."""
    return Path(settings.rag_eval_baseline_path)


@router.get("/rag/eval/status", response_model=RagEvalStatusResponse)
def rag_eval_status() -> RagEvalStatusResponse:
    """RAG 评估套件的现状（day071）：评测集规模 + 判据 + 变体表 + 指标口径 + 基线.

    它回答的是"**这次评估是在什么条件下做的**"，而这恰好是读报告时最先需要的东西：

```text
suite      评测集 6 条、语料 6 条、k=3、覆盖率下限 0.5 —— 口径
variants   四个变体各自"只改了哪一项"（baseline / prompt-v1 / cite-gate / tight-budget）
metrics    11 个指标的 {name, direction, description} —— 数字往哪个方向算好
tags       12 类坏例标签的 {description, action} —— 坏在哪一层、下一步做什么
baseline   已提交的那份质量基线（不存在时是 **null**，而不是一份全零的假基线）
```

    最后一行是刻意的：**没有基线**与"基线全是零"是两回事（前者该去跑一次采集，
    后者会让所有回归判定静默失效）。因此这里宁可给 ``null``，也不兜底造一份。

    本端点**不跑任何用例**（0 次模型调用、0 次检索）：它是"看一眼配置"的入口，
    真正要跑请用 ``POST /rag/eval/run``。
    """
    suite = build_rag_eval_suite()
    baseline_path = rag_eval_baseline_file()
    baseline = (
        RagBaseline.load(baseline_path).to_dict() if baseline_path.exists() else None
    )
    described = suite.describe()
    return RagEvalStatusResponse(
        summary=(
            f"套件 {described['label']}：评测集 {described['cases']} 条、"
            f"语料 {described['corpus']} 条、k={described['k']}、"
            f"覆盖率下限 {described['min_coverage']}、"
            f"变体 {len(described['variants'])} 个"
        ),
        suite=described,
        defaults=rag_eval_defaults(),
        variants=[item.to_dict() for item in suite.variants],
...
@router.post("/rag/eval/run", response_model=RagEvalRunResponse)
def rag_eval_run(req: RagEvalRunRequest, request: Request) -> RagEvalRunResponse:
    """跑一次端到端 RAG 评估（day071）：报告 + 基线 + 与已提交基线的对比.

    它跑的是**当前 app 注入的那个库**（``app.state.vector_store`` +
    ``app.state.embedding``）与 ``app.state.llm``：因此"评估的对象"与
    "端点实际服务的那个系统"是同一套东西——这正是 day046 的
    ``/pipeline/baseline`` 立下的那条纪律（拿另一个库评出来的数字不能代表线上）。

    三处值得先读清楚的行为：

```text
空库             → 200，报告里每条用例都归因成 no_data（**不是 500**）：
                 "还没灌语料"与"检索坏了"是两件事，而报告会如实说前者
基线不存在        → comparison=null（**不造一份全零基线**，见 /rag/eval/status）
基线与当前不可比   → comparison 里 conclusive=false + reasons 逐条说明
                 （k 口径变了 / 索引版本变了 / 样本不足），**不报假回归**
```

    ``judge=true`` 会用**同一个** ``app.state.llm`` 充当评审模型（每条用例多一次
    调用）：把评审与作答交给两个不同模型在生产上更稳（与被评模型同盲区是
    评审的已知风险，见 day027），但在离线演示里"一个确定的替身"更可复现。

    参数非法一律 **400**（变体名不存在、请求体形状不对、评测集缺字段）；
    缺 ``variant`` 字段是合法的（默认跑变体表里的第一个）。
    """
    try:
        suite = build_rag_eval_suite()
        ...
        report = suite.run_variant(
            ...
            llm,
            req.variant,
            judge=llm if req.judge else None,
        )
        baseline = report.to_baseline()
        comparison = None
        baseline_path = rag_eval_baseline_file()
        if req.compare_with_baseline and baseline_path.exists():
            comparison = guard_from_settings(RagBaseline.load(baseline_path)).compare(
                baseline
            ).to_dict()
    ...
    except (RagDebugError, RetrievalError) as exc:
        # 两族异常合成一条通道：对本层的调用方而言，它们都是"这次评估没做成、
        # 消息里写着为什么"（变体名写错 / 评测集缺字段 / 没配 LLM / 基线与当前不可比）。
        raise HTTPException(status_code=400, detail=str(exc)) from exc


@router.get("/retrieval/prompt/versions", response_model=PromptVersionsResponse)
def retrieval_prompt_versions() -> PromptVersionsResponse:
    """受管提示词的清单与元信息（day069）：**纯读端点**，不检索、不调模型、不写状态.

    它存在的理由是**归因顺序**。读到一份"换了一版提示词之后结果不对"的报告时，
    要按这个顺序排除：

    ```text
    1. 现在生效的是哪一版        → current（CURRENT_PROMPT_VERSION）
    2. 没指定版本号时用哪一版    → default（settings.retrieval_prompt_version）
    3. 这一版比上一版改了什么    → templates 的 chars / changelog / constraints
    4. 六种"不算数"分别是什么    → fallback_reasons（取值 + 人话解释 + 出路）
    5. "资料不足"要怎么说        → decline_markers（检测清单与 v2 的约束 2 是同一份）
    ```

    第 1、2 两行**刻意分开**：它们今天相同（都是 ``v2``）但服务两个问题——
    前者回答"现在生效的模板是哪一份"，后者回答"没指定时用哪一版"。
    换版本时只改前者；改后者是一次会让所有调用方一起换模板的显式决定。

    第 5 行的三句话是**封闭清单**：它既是 v2 要求模型写的句式，也是
    ``generation._declined`` 的检测清单——两者一旦分家，"模型自己说没有相关内容"
    会变成一件有时能检测、有时检测不到的事，而那种不一致在报告里看不出来。

    它**不做什么**：不生成、不检索、不校验请求体（没有请求参数）、
    不落盘、不改任何状态；也**不代表生产配置**——这里端出来的是这台机器上
    那一份受管清单与它当前的缺省值（要注入自定义模板请用
    ``RAGGenerator(prompt=...)``，那条路只能在装配期走）。
    """
    return PromptVersionsResponse(
        summary=(
            f"受管提示词 {len(PROMPT_VERSIONS)} 版（当前 {CURRENT_PROMPT_VERSION}，"
            f"缺省 {DEFAULT_PROMPT_VERSION}）"
            f" | 固定拒答句式 {len(DECLINE_MARKERS)} 条"
            f" | 回退原因 {len(FALLBACK_REASONS)} 种（另有空串表示'没有回退'）"
        ),
        versions=list(PROMPT_VERSIONS),
        current=CURRENT_PROMPT_VERSION,
        default=DEFAULT_PROMPT_VERSION,
        templates=prompt_templates(),
        # 六个取值**一个不少**（含表示"没有回退"的空串）：这份表的用途是"按原因
        # 分组统计"，少一个取值就意味着有一批结果无法归组，而它们看起来像"未知原因"。
        fallback_reasons=[
            {"reason": reason, "description": FALLBACK_REASON_DESCRIPTIONS[reason]}
            for reason in (FALLBACK_REASON_NONE, *FALLBACK_REASONS)
        ],
        decline_markers=list(DECLINE_MARKERS),
        defaults=generation_defaults(),
        limitations=list(RETRIEVAL_LIMITATIONS),
...
@router.post("/retrieval/grounding/verify", response_model=GroundingVerifyResponse)
def retrieval_grounding_verify(req: GroundingVerifyRequest) -> GroundingVerifyResponse:
    """一段答案 + 一份编号表 → 引用核对（day069）：**本组唯一不调模型的端点**.

    它是"接地校验"最纯粹的一层：没有检索、没有打包、没有提示词渲染，也没有任何一次
    模型调用。因此它是排查时的第一站——"这段答案里有没有幻觉引用"这个结论可以
    独立复算，而不必先跑一遍生成（那要花钱、要联网，还要有一个可用的模型）。

    六个数字一起才说得出结论（定义见 ``GroundingReport``）：

    ```text
    given     这次给了几条片段（= len(valid) + len(unused)，算出来的）
    cited     答案里出现过的编号（升序去重）—— 来自答案
    valid     cited 里真的落在编号表里的那些       → 覆盖率的分子
    invalid   cited 里落不到编号表里的那些         → 幻觉引用，最该被看见的那个数
    unused    给了它、答案却没引用的编号           → 覆盖率的分母
    coverage  len(valid) / given（4 位小数）
    ```

    三条边界行为，每一条都对应一个不同的动作：

    ```text
    全部合法（valid 非空且 invalid 为空）→ grounded=True：**有可核对的依据**
                                          （注意它不等于"这句话忠实于片段"）
    有幻觉引用（invalid 非空）          → grounded=False，invalid 里点名是哪几个编号，
                                          幻觉的那几行 record_id 为 None
    一个 [n] 都没有                     → covered 0，notes 写明"这是没有引用，
                                          与'引用了不存在的编号'是两件事"
    ```

    参数非法一律 **400**：``marker < 1``（编号从 1 起，``[0]`` 不是编号）与空
    ``record_id`` 由 ``Citation`` 判定，消息里说明"没有 id 的引用无法回库取原文"。
    **缺字段（没给 answer / citations）是 422**：那是契约问题而不是核对参数问题
    （形状错在 pydantic 那一层就被拦下，业务代码根本看不到它）。

    它**不做什么**：不调任何模型（下面的用例用一个会计数的假 LLM 证明这一点：
    它的 ``calls`` 恒为 0）、不读任何注入对象（连库与编码器都不需要）、
    不检索、不打包、不落盘；也**不做语义判断**——它只对账编号，
    "这句话是不是被片段支持"要读片段内容，那一层不在本端点（口径见
    ``GroundingReport`` 的说明：它叫 grounded 而不是 correct）。
    """
    try:
        # 请求体里的编号表 → 领域对象 Citation：这里只搬字段，**校验不在这里**
        # （编号从 1 起、record_id 不能是空串都由 Citation.__post_init__ 判，
        # 消息也由它给）。端点上再判一遍的后果是两份规则迟早分家，
        # 而分家的表现是"某个写法在生成链路里被接受、在这里被拒"。
        citations = tuple(
            Citation(
                marker=item.marker,
        ...
        # 核对用的是**生成链路那一份实现**（见文件头那行"按模块名导入"的说明）：
        # 在端点里再写一遍"解析 [n] 再分三类"不会报错，只会在某一天让两个结论
        # 分家（端点说"没有幻觉引用"、生成器说"有一条"），而分家没有报错指向它。
        report: GroundingReport = generation_module._grounding_report(req.answer, citations)
    except RetrievalError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    return GroundingVerifyResponse(
        summary=report.summary_line(),
        given=report.given,
        cited=list(report.cited),
        valid=list(report.valid),
        invalid=list(report.invalid),
        unused=list(report.unused),
        coverage=report.coverage,
        grounded=report.grounded,
        hallucinated=report.hallucinated,
        checks=[row.to_dict() for row in report.checks],
...
# --------------------------------------------------------------------------- #
# RAG 生产化运维（day072 / M6-D10）：status / health / sync
# --------------------------------------------------------------------------- #


def rag_ops_health_rules() -> list[str]:
    """健康检查项的口径（端点回显它，而不是让调用方去读源码）.

    与 ``rag_eval_defaults()`` 同一取向：**判据本身也要能被读出来**。
    一个只知道"返回值里有 ok / warn / fail"的调用方，
    无法回答"这个实例到底查过哪几项"。
    """
    return [f"{name}: {HEALTH_CHECK_DESCRIPTIONS[name]}" for name in HEALTH_CHECKS]


@router.get("/rag/ops/status", response_model=RagOpsStatusResponse)
def rag_ops_status(request: Request) -> RagOpsStatusResponse:
    """RAG 运维现状（day072）：调度判定 + 索引现状 + 监控 + 编排校验（**只读**）.

    它把四块本来分散在四个地方的账放在一起，因为运维问的从来不是一个孤立的问题：

    ```text
    "今天同步了吗"        → schedule（账本 + 判定 + 下一次在哪）
    "库里现在有多少条"     → index（day065 的 stats）
    "最近量到了什么"       → metrics（本进程内的采样汇总）
    "部署配置还对得上吗"   → deploy（11 条静态判据 + 与仓库 yml 的一致性）
    ```

    **0 次模型调用、0 次检索、0 次写盘**：整条路径只读文件与内存里的计数器，
    因此它可以被探针、看板与人工反复调用。
    """
    pipeline = rag_ops_pipeline(request)
    snapshot = pipeline.status()
    index_stats = snapshot["index"]
    deploy_state = snapshot["deploy"]
    problems = len(deploy_state["problems"])
    deploy_summary = "合规" if deploy_state["valid"] else f"有 {problems} 个问题"
    return RagOpsStatusResponse(
        summary=(
            f"{snapshot['schedule']['decision_summary']} | 库 {index_stats['count']} 条 | "
            f"编排 {deploy_summary}"
        ),
...
@router.get("/rag/ops/health", response_model=RagOpsHealthResponse)
def rag_ops_health(request: Request, response: Response) -> RagOpsHealthResponse:
    """RAG 实例健康（day072）：四项检查 + 三态 + 探针退出码.

    **本端点是给容器探针用的**（``docker-compose.rag.yml`` 里那句
    ``curl -f http://localhost:8080/rag/ops/health``），因此它有一处
    刻意的不对称：``state == "fail"`` 时 HTTP 状态码是 **503**。

    ```text
    200   可以继续接收流量（ok 与 warn 都算"活着"）
    503   这个实例现在答不了知识库问题（库空 / 清单与库不符 / 质量判定回归）
    ```

    如果永远返回 200，``curl -f`` 永远成功——那等于没有探针；而"把状态放在
    响应体里让人自己看"在自动化编排里不是一条可用的约定。
    响应体与状态码**同时**给出结论：看板读状态码，人读响应体里的 4 条判据。

    它**不跑评估、不扫语料**（见 ``rag_ops.health`` 模块的第 1 条纪律）：
    四项检查的输入全部是"别人已经算好的账"。``quality_regression`` 缺输入时
    给 ``warn`` 而不是 ``ok``——"没评估"永远不等于"评估通过"。
    """
    pipeline = rag_ops_pipeline(request)
    report = pipeline.health_report()
    if report.state == STATE_FAIL:
        response.status_code = 503
    return RagOpsHealthResponse(
        summary=report.summary_line(),
        state=report.state,
        ok=report.ok,
        ...
        rules=rag_ops_health_rules(),
    )


@router.post("/rag/ops/sync", response_model=RagOpsSyncResponse)
def rag_ops_sync(request: Request, req: RagOpsSyncRequest) -> RagOpsSyncResponse:
    """跑一次 RAG 语料同步（day072）：扫盘 → 差集 → 重建 → 体检 → 落账.

    **本端点的产物是一份报告，不是一个业务结果**，因此它的状态码只有两档：

    ```text
    200  本次运行有结论：not_due / no_change / synced / failed 四取一，
         `report.status` 如实报出——包括失败（报告里带着原因与它前面的每一步）
    400  请求本身不可执行（例：语料目录不存在、参数非法）
    503  这个实例没有装配运维流水线（见 rag_ops_pipeline）
    ```

    为什么失败也给 200：四档状态塞进 HTTP 状态码会让"没到点"与"同步失败"
    在监控里长得一样，而它们的处置完全不同（前者什么都不用做，后者要有人去看）。
    真正的"跑不了"（目录不存在、编码器缺失）仍然走 400 —— 那才是调用方要改的东西。

    ``force=true`` 会绕过调度判定（间隔与退避），用于"我就是要现在跑一次"；
    默认 ``false`` 时，若未到计划时间会得到一份 ``status=not_due`` 的报告，
    而它**不覆盖**上一次落盘的报告（见 ``RagOpsPipeline._finish`` 的说明）。
    """
    pipeline = rag_ops_pipeline(request)
    try:
        report = pipeline.run_once(force=req.force)
    except RagOpsError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    return RagOpsSyncResponse(
        summary=report.summary_line(),
        status=report.status,
        report=report.to_dict(),
        notes=list(report.notes),
        persisted={
            "ledger": pipeline.ledger_path,
            "report": pipeline.report_path,
            "note": "空串表示本次不落盘（rag_ops_dir 为空）",
        },
    )
