...
class TestTopCostPaths:
    def test_groups_by_model_and_endpoint(self):
        tracker = CostTracker()
        tracker.record("gpt-4o-mini", 1000, 500, endpoint="chat")
        tracker.record("gpt-4o-mini", 2000, 0, endpoint="embedding")
        tracker.record("gpt-4o", 500, 500, endpoint="chat")
        paths = tracker.top_cost_paths()
        # 三个「模型 × 接口」组合各自成行
        assert len(paths) == 3
        keys = {(p["model"], p["endpoint"]) for p in paths}
        assert keys == {
            ("gpt-4o-mini", "chat"),
    ...
    def test_sorted_by_cost_desc(self):
        tracker = CostTracker()
        # gpt-4o 单价高，同量级 token 下费用最高，应排第一
        tracker.record("gpt-4o", 1000, 1000, endpoint="chat")
        tracker.record("gpt-4o-mini", 1000, 1000, endpoint="chat")
        paths = tracker.top_cost_paths()
        assert paths[0]["model"] == "gpt-4o"
        assert paths[1]["model"] == "gpt-4o-mini"
        assert paths[0]["cost_usd"] > paths[1]["cost_usd"]

    def test_top_n_limits_results(self):
        tracker = CostTracker()
        for i in range(4):
            tracker.record("gpt-4o-mini", 100 + i, 0, endpoint=f"ep{i}")
        assert len(tracker.top_cost_paths(top_n=2)) == 2

    def test_report_includes_top_paths(self):
        tracker = CostTracker()
        tracker.record("gpt-4o", 1000, 1000, endpoint="chat")
        report = tracker.report()
        assert "top_paths" in report
        assert report["top_paths"][0]["model"] == "gpt-4o"

    def test_empty_tracker_top_paths(self):
        assert CostTracker().top_cost_paths() == []
