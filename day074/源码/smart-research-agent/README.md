...
M6 - 检索增强（**进行到 day069 / M6-D8**）：day066 单路检索 → day067 混合检索（BM25 + 融合）
→ day068 重排序（交叉编码器只对前 N 条打分）→ day069 RAG 生成器与 Prompt（提示词 v2 四段式 +
引用溯源 + 接地校验 + 六种回退）。生成器只做**编号层面**的核对
（`grounded = valid 非空 且 invalid 为空`），换提示词版本会改变答案文本本身；
`retrieval_require_citation` 是**缺省关闭**的一道闸门。
...
- 这一层的边界与它明确不做的事写在 `types.RETRIEVAL_LIMITATIONS` /
  ...
  不重排序（day068）、`where` 是元数据筛选而不是权限"——**前两条都已经兑现**
  （见下面两节），因此现在那两份清单里剩下的是"重排只作用于前 N 条窗口"、
  "教学级交叉编码器替身没有真实语义"、"阈值只对向量通道生效"与
  "关键词一路用 2-gram 代替分词器"这类仍然成立的边界。
...
day066 回答了"取回哪几条"、day067 回答了"两路怎么合"。day068 补上最后一段：
**这一批候选里，哪一条真的最相关**。双塔把查询与文档**分开**编码（文档侧可预计算，
所以能扫百万条），代价是两边**从来没见面**——编码器没见过的东西（编号、函数名、报错码）
只能"差不多就行"；交叉编码器把 `[CLS] 查询 [SEP] 文档` 一起过一遍模型，更准，也贵得多
（每对一次前向推理）。因此标准姿势是两段式：**第一阶段召回别漏，第二阶段重排排准**。

```text
第一阶段（召回）→ 候选 N 条 → 切窗口（前 top_n 条）→ 交叉编码器逐对打分
  → [blend：窗口内两份分数各自 min-max 归一化后加权] → 阈值落刀（只切被打分过的那批）
  → 重排名次（窗口内按重排分）+ 原顺序尾部 → 多样性裁剪 → 截断到 top_k → 诊断与装配
```

| 模块 | 回答什么 | 关键符号 |
|------|---------|---------|
| `retrieval/rerank.py` | 交叉编码器替身、窗口纪律、两种模式 | `rerank_hits` / `CrossEncoderReranker` / `RerankResult` / `RerankHit` / `RerankFeatures` |
| `retrieval/rerank.py`（评估） | 重排到底有没有用 | `LiftProbe` / `LiftReport` / `measure_lift` / `recall_at_k` / `reciprocal_rank` / `ndcg_at_k` |
| `retrieval/retriever.py` | 单路第 6.5 步（阈值后、多样性前） | `Retriever._apply_rerank` / `_rebuild_hits` / `_renumber` |
| `retrieval/hybrid.py` | 混合第 10.5 步（融合后、多样性前） | `HybridRetriever._resolve_rerank` / `_apply_rerank` |

- **教学级替身，诚实写在名字里**：`cross-encoder-teaching-v1` **没有真实语义**（认不出
  同义改写、读不懂语序），分数由四个可手算特征加权而成
  （`FEATURE_WEIGHTS = (0.5, 0.25, 0.125, 0.125)`，和**逐位等于 1.0**，因此 `min_score`
  在这个 `[0, 1]` 的尺度上是可标定的）；接真实模型只要写一个 `BaseReranker` 子类，
  本模块其余部分不需要改动；
- **窗口纪律**：只对前 `top_n=20` 条打分，窗口外一条都不打分（`scored=False`、
  `score=0.0`——不是"最不相关"，而是"没有分"），这条注记**无条件**进 `notes`；
- **两种模式**：`replace`（缺省，窗口内只按重排分）与 `blend`（窗口内两份分数**各自**
  min-max 归一化后按 `weight=0.5` 加权）；跨阶段的分数量纲不可比，这一步归一化不是可选项；
- **阈值与第四道减法**：`min_score` 比的是**重排分**且**只作用于被打分过的条**，
  切掉几条记进 `dropped_by_rerank`（第四个 `dropped_*`，**不并进** `dropped_by_top_k`）；
- **重排分与第一阶段分是两个口径**：`score` 保持不变（第一阶段的分数），重排分写在
  `rerank_score`、重排前的位置写在 `stage1_rank`——因此重排过的名单里 `score` **不再单调**；
- **排序键三个键**：`(-score, stage1_rank, record_id)`（blend 是 `(-blended, …)`）——
  并列由第二键（输入位置）决定，第三键是防御性兜底；
- **缺省关闭**（`retrieval_rerank_enabled=False`）：关着时第 6.5 / 10.5 步什么都不做、
  一个字段都不改写，day066/day067 的结果**逐位不变**。

```bash
# 一键演示（九节：两列次序对照 → 窗口纪律 → 四特征数值 → replace/blend 手算
#              → 阈值落刀 → 排序确定性 → 评估提升 → 单路/混合两种集成 → 关闭重排的对照）
python scripts/rerank_demo.py
# 产出 outputs/rerank_demo.txt（本机实测：ERR-2043 的第一阶段第 1 名一个编号都不含，
# 重排把真正含它的那三条提到前三；3 条探针的 recall / RR / nDCG 从 0.5 / 0.4417 / 0.4124
# 涨到 1.0 / 1.0 / 1.0）
```

`config.py` 新增 6 项（`retrieval_rerank_enabled` / `retrieval_rerank_model` /
`retrieval_rerank_top_n` / `retrieval_rerank_weight` / `retrieval_rerank_mode` /
`retrieval_rerank_min_score`），与既有 `retrieval_*` 同属"只改变怎么问"那一组，
同样遵守 day067 那条**"新能力默认不生效"**。手册（四个特征与权重、窗口纪律的代价、
两种模式与量纲、阈值与第四道减法、结果证据的读法、三条提升指标、参数覆盖、
端点清单、常见误区 12 条）见 [`docs/reranking.md`](docs/reranking.md)。

## RAG 生成器与 Prompt（day069）

day066 回答了"取回哪几条"、day067 回答"两路怎么合"、day068 回答"哪条最相关"。day069 补上链条
末端一直没人管的那一步：**模型给的那段话，能不能被片段核对**。它要拦的是 RAG 最贵的一种失败
——调用成功、答案通顺、引用格式也像模像样，但内容全部来自预训练记忆，而读的人看不出区别。

```text
渲染提示词（v2 四段式）→ 调一次模型 → 解析答案里的 [n] → 与那次提示词的编号对账
  → 折成三个数字（有效 / 幻觉 / 未引用）+ 一个判定（grounded = valid 非空 且 invalid 为空）
  → 六种回退（no_context / llm_error / empty_reply / model_declined / unusable_citations / 无）
```

| 模块 | 回答什么 | 关键符号 |
|------|---------|---------|
| `retrieval/generation.py` | 怎么问、答案算不算数 | `RAGGenerator` / `Generation` / `GroundingReport` / `CitationCheck` / `FALLBACK_REASONS` / `DECLINE_MARKERS` |
| `retrieval/generation.py`（提示词） | 模型被怎么问的 | `RAG_PROMPT_V1` / `RAG_PROMPT_V2` / `PROMPT_VERSIONS` / `PROMPT_CHANGELOG` / `prompt_template_of` |
| `retrieval/pipeline.py` | 检索 → 打包 → 生成 → 引用 | `RagPipeline.answer` / `RagAnswer.check` / `RagAnswer.fallback_reason`（**只增不改**） |
| `retrieval/errors.py` | 生成参数写错怎么报 | `GenerationError`（继承 `QueryError`："改这一次的调用点或装配"） |

- **它只做编号层面的核对**：答案里的 `[n]` 与**那一次提示词**的编号一一对账（有效 / 幻觉 /
  未引用），折成 `coverage = len(valid) / 给出去的片段数`（4 位小数）。**不做语义忠实性判断**，
  也不做答案质量评估（那是 day071）；
- **`grounded` 不是字符串比较**：`valid 非空 且 invalid 为空`。换成"看答案里有没有'资料中没有
  相关内容'"，兜底文案改一个字就静默失效（而它失效的样子恰好是"看起来一切正常"）；
- **`[0]` 不是编号**（编号从 1 起）：它既不算引用、也不算幻觉引用，只进报告自己的注记；
- **提示词 v1 → v2**：四段式（资料片段 / 问题 / 约束 / 输出格式）+ 约束从 4 条扩到 6 条 +
  **固定拒答句式**（`DECLINE_MARKERS` 三条）——句式固定之后，"模型自己说没有"从一句无法检测的
  话变成一件可检测的事实。历史版本永不删，`prompt_template_of` 只认受管清单；
- **六种回退各自对应一个处置动作**（不是一个布尔标志）：`no_context`（检索 / 打包为空，
  **一次 LLM 都不调**，`llm_called=False`）/ `llm_error`（调过了、调用抛异常）/ `empty_reply`
  （调过了、返回空串）/ `model_declined`（命中固定句式）/ `unusable_citations`
  （`require_citation=True` 且没有合法引用）/ 空串（正常路径，答案来自模型且核对出有效引用）；
- **护栏有两个把手**：`pipeline` 管"检索为空"（`empty_reason` 四种），生成器管"打包为空"
  （`PackedContext.is_empty`）——两处都不许把"没有片段"变成"让模型自由发挥"，因此判空排在
  **渲染提示词之前**（空上下文时 `prompt_text == ""`、`llm_called=False`）；
- **`require_citation` 缺省关闭**（一道闸门，不是阈值）：打开之后，模型只是忘了写 `[n]` 就会被
  整条丢掉（答案换兜底答复），因此先让默认可核对率跑一段时间（day071 会给出那个数字）；
- **三份证据的读法只定义一次**：`Generation.to_dict()` 默认带上 `prompt_text`（"模型到底被怎么
  问的"），而 `to_summary()` 刻意不含它（摘要是进上游公共字段的一小段）；`RagAnswer.to_dict()`
  尾部只增 `check` / `fallback_reason` 两个键，老消费者读到的键一个都不少也不改。

```bash
# 一键演示（九节：v1/v2 模板对照 → 正常生成 → 引用三态 → coverage 扫描表 → 拒答句式 + 反例
#              → 空回复 / llm_error → require_citation 四格 → 空上下文早退 → pipeline 集成对照）
PYTHONPATH=. python scripts/generation_demo.py
# 产出 outputs/generation_demo.txt（本机实测：给了 6 条片段、答案引用 2 条 → coverage 0.3333、
# grounded=True；换成越界 [9] 时 invalid=[9]、grounded=False；空上下文时 llm.calls == 0）
```

`config.py` 新增 4 项（`retrieval_prompt_version` / `retrieval_answer_temperature` /
`retrieval_answer_max_tokens` / `retrieval_require_citation`），与既有 `retrieval_*` 同属
"只改变怎么问"那一组，但比前两组**重一档**：换提示词版本会**改变答案文本本身**（同一份索引、
同一个问题，v1 与 v2 交给用户的是两段不同的话），因此它是一次需要留痕的实验。手册（九步
流水线、v1 → v2 的四段式、引用三个结局、接地判定、六种回退、`grounded`/`coverage` 的扫描表、
参数覆盖、端点清单、常见误区 12 条）见
[`docs/rag_generation.md`](docs/rag_generation.md)。
