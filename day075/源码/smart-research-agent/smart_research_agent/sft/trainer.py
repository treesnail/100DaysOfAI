...
class SFTTrainer:
    ...
    def __init__(
        ...
        *,
        ...
        on_step: Callable[[int, Any, Sequence[StepRecord]], None] | None = None,
    ):
        self.model = model
        ...
        #: 每一步参数更新之后的回调 ``(global_step, model, records)``（day052 起）.
        #:
        #: 存在的理由：有了 LoRA，"每 N 步落一次盘"的语义变了——全参微调要存
        #: 整份权重，而 LoRA 只要存两个小矩阵（外加一份训练状态）。这个动作
        #: 必须发生在**训练循环内部**（否则拿不到中间步的适配器），而循环本身
        #: 没有任何理由再写一遍。所以这里只开一个钩子，**循环保持一份**：
        #: ``scripts`` 一侧的回调负责落盘，训练逻辑一行不复制。
        #:
        #: 回调抛异常会让训练整体失败——这是刻意的：落盘失败（磁盘满、权限）
        #: 属于必须立刻中止的情况，静默跳过会让"以为存了"变成一颗雷。
        ...
        while global_step < total_steps:
            ...
            for batch in order:
                ...
                if self.on_step is not None:
                    # 回调放在"记录已经写进 records、间隔落盘也已经做过"之后：
                    # 它看到的是一个完整一致的快照（step + records + 权重）。
