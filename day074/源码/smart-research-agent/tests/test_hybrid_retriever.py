...
class TestFusionParameters:
    ...
    @pytest.mark.parametrize("key", ["alhpa", "fusion", "weigth", "strategy_", ""])
    def test_unknown_extra_keys_are_rejected(self, key: str) -> None:
        """静默忽略一个覆盖参数会让调用方以为它生效了（与 RagPipeline 同一条纪律）.

        day068 改掉了一个参数值：``"weight"`` 原本是错键，但重排层的封闭清单里
        真的有它（``RERANK_OVERRIDE_KEYS``），因此这里换成拼错的 ``"weigth"``——
        这条用例要守的是"**清单外的键必须响**"，而不是某个特定的字符串。
        """
