...
class TestStepCallback:
    """``on_step`` 回调（day052）：训练循环里唯一的扩展点.

    LoRA 要"每 N 步落一次适配器"，而这个动作必须发生在**循环内部**（否则拿不到
    中间步的适配器），而循环本身没有任何理由再写一遍——所以这里只开一个钩子，
    训练逻辑保持一份。三条用例把它钉住：调用次数与步号、回调看到的 ``records``
    是**完整快照**（这一步的记录已经写进去）、回调抛异常必须让训练整体失败
    （落盘失败属于必须立刻中止的情况，静默跳过会让"以为存了"变成一颗雷）。
    """

    #: 区分"显式传 ``on_step=None``"与"根本不传 ``on_step``"（后者走缺省值）
    _ABSENT = object()

    def _train(self, tmp_path, data, *, on_step=_ABSENT) -> SFTReport:
        """按给定回调跑一次训练；``on_step`` 缺省时**不把它传给构造器**."""
        _, _, tokenizer = data
        extra = {} if on_step is self._ABSENT else {"on_step": on_step}
        trainer = SFTTrainer(
            ReferenceSFTModel(tokenizer.vocab_size, seed=42),
            tokenizer,
            ref_args(tmp_path),
        ...
        return trainer.fit(data[0], data[1], save_at_end=False)

    def test_called_once_per_step_with_growing_records(self, tmp_path, data):
        """每次参数更新之后调用一次；``step`` 从 1 递增，``records`` 长度 == step."""
        seen: list[tuple[int, int]] = []

        def on_step(step: int, model, records) -> None:
            seen.append((step, len(records)))

        report = self._train(tmp_path, data, on_step=on_step)
        assert report.total_steps == 12
        assert [step for step, _ in seen] == list(range(1, 13))
        assert [count for _, count in seen] == list(range(1, 13))

    def test_callback_error_fails_the_whole_fit(self, tmp_path, data):
        """回调抛异常必须冒泡出去——落盘失败不能静默跳过."""

        def on_step(step: int, model, records) -> None:
            raise RuntimeError("落盘失败：磁盘已满")

        with pytest.raises(RuntimeError, match="落盘失败"):
            self._train(tmp_path, data, on_step=on_step)

    def test_none_callback_keeps_default_behaviour(self, tmp_path, data):
        """不传 ``on_step``（None）时行为与之前完全一致：loss 序列逐位相同."""
        explicit_none = self._train(tmp_path / "explicit", data, on_step=None)
        implicit_none = self._train(tmp_path / "implicit", data)

        def noop(step: int, model, records) -> None:
            pass

        noop_callback = self._train(tmp_path / "noop", data, on_step=noop)

        baseline = [record.loss for record in implicit_none.records]
        assert [record.loss for record in explicit_none.records] == baseline
        # 一个什么都不做的回调同样不该扰动训练
        assert [record.loss for record in noop_callback.records] == baseline
        assert explicit_none.optimizer_steps == implicit_none.optimizer_steps == 12
