...
from smart_research_agent.evaluation.harness import CaseResult
...
class EchoHarness(EvaluationHarness):
    """day026 简版用例（保留）：最简具体实现，expected 是 input 的子串则通过.

    适配说明：canonical 的 ``summary`` 是实例方法，需以 ``EchoHarness().summary(results)``
    实例调用（day026 原始简版中是类身份调用的静态方法）。
    """
