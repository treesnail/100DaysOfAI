...
class CostTracker:
    ...
    def top_cost_paths(self, top_n: int = 5) -> list[dict]:
        ...
        为什么「路径」要组合模型与接口，而不是只看其一：同一个模型走
        embedding 接口（按输入 token 计费）与走 chat 接口（输入输出都计费）
        的成本结构完全不同，只看模型或只看接口都会漏掉「贵在哪条链路」。
        组合两者排序后，最烧钱的路径一目了然，成本优化从这里下手（例如
        给高频路径加缓存、换更便宜的模型、压缩提示词）。
        """
    ...
    def report(self) -> dict:
        """导出聚合报告：总览 + 按模型 + 按接口分解 + 高成本路径（day043）.

        可直接 json.dumps。``top_paths`` 是 day043 新增的高成本调用路径
        识别：把「模型 × 接口」归因后按费用降序，供成本优化优先处理。
        """
        return {
