...
class IndexingPipeline:
    ...
    def versions(self) -> IndexVersionStore | None:
        ...
        day072 的 ``rag_ops`` 需要它做两件事：体检时读"库里那批记录对应哪份清单"，
        同步成功后把版本表落盘。两者都**必须**用构建器手里那一个对象——

        ```text
        另建一个 IndexVersionStore(同一个目录)
            构建器 register 到它自己内存里的表 → 运维层那份仍然空着
            → current_manifest() 永远返回 None（"清单丢了"），而磁盘上其实有
        ```

        这类 bug 不报错、只让体检的结论偏保守，因此把"版本表只有一个来源"
        变成结构上的必然，而不是靠两处代码记得同步。

        ``None`` 表示这条流水线没有接版本表（见构造器说明）——它是合法状态，
        不是异常：不接版本表的构建仍然能跑，只是没有历史和备份。
        """
