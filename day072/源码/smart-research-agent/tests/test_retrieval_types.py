...
class TestConstants:
    ...
    def test_limitations_and_out_of_scope_are_written_down(self) -> None:
        """两份"边界声明"必须写明与 day069 的分工（写下来才不算 bug）.

        day067 改写过这一条：BM25 与融合**已经落地**（``lexical`` / ``fusion`` /
        ``hybrid``），因此"不实现 BM25"不再是本层的限制——反过来断言它不在里面，
        是为了防止这一段边界声明在实现之后忘记更新（那会让文档与代码互相矛盾，
        而"文档说没实现、代码里已经能用"是最容易误导下一位读者的一种漂移）。
        day069 第三次改写沿用同一条纪律：引用核对与接地判定落地之后，
        "不做引用溯源校验"同样必须从边界里消失，换上来的两条是兑现之后
        **仍然成立**的边界（只做编号层面的核对 / 生成器只做推理侧）。
        """
        assert len(RETRIEVAL_LIMITATIONS) >= 3
        assert all(isinstance(item, str) and item.strip() for item in RETRIEVAL_LIMITATIONS)
        joined = " ".join(RETRIEVAL_LIMITATIONS)
        assert "day068" in joined
        assert "重排" in joined
        assert "day069" in joined
        assert "接地" in joined
        # 关键物**不在**限制里了：day067 已实现 BM25 与融合、day069 已实现引用核对。
        assert "不实现 BM25" not in joined
        assert "不做引用溯源" not in joined
        assert "day067" not in joined
        # 但"关键词一路用 2-gram 代替分词器"这条**真实**的边界必须还在（不许静默消失）。
        assert "2-gram" in joined

        assert len(RETRIEVAL_OUT_OF_SCOPE) >= 3
        assert "查询改写" in " ".join(RETRIEVAL_OUT_OF_SCOPE)
        # day069 明确写下的那一句：答案质量评估是别人那一天的活（day071）。
        assert "day071" in " ".join(RETRIEVAL_OUT_OF_SCOPE)
...
class TestRetrievalHit:
    ...
    def test_to_dict_key_set(self) -> None:
        ...
        assert set(payload) == {
            ...
            "char_count",
            "channel",
            # day067 新增：多路证据（``()`` = 未记录，即 day066 的单路语义）。
            "channels",
...
class TestRetrievalResultContract:
    ...
    def test_to_dict_key_set(self) -> None:
        ...
        assert set(payload) == {
            ...
            "dropped_by_diversity",
            "dropped_by_top_k",
            # day068 新增：第四个数（重排阈值）与重排摘要（空 = 这次没开重排）。
            "dropped_by_rerank",
            "empty_reason",
            "metric",
            "index",
            "latency_ms",
            "channels",
            # day067 新增的两项多路证据（空字典 = 这次是单路检索，没有多路证据可记）。
            "channel_candidates",
            "fusion",
            "rerank",
            "doc_ids",
