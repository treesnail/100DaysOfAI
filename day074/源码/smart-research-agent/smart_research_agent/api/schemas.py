...
class RetrievalAnswerResponse(BaseModel):
    ...
    day069 给本响应**只增不改**地补了两个键（照 day068 给 ``RetrievalHit`` 加
    ``rerank_score`` 的先例，两个键都加在末尾）：

    ```text
    check           GroundingReport 的投影：答案里的 [n] 与那次提示词的编号
                    一一对账的结果（三组编号 + 覆盖率 + 接地）。空检索那条路
                    一次都没走到生成，因此它是 None——"没有核对可言"而不是
                    "核对没通过"（后者的三组编号都在）
    fallback_reason 六取一：这次答案为什么不能按正常路径读（空串 = 没有回退）。
                    与 generation.FALLBACK_REASON_DESCRIPTIONS 是同一份闭清单
    ```

    因此老消费者读到的键一个都不少、值一个都不变：这两个键是**新增的**，
    而它们回答的正是"这份答案有没有依据"这个 day069 才开始可回答的问题。
    """
    ...
    route: dict = Field(default_factory=dict, description="选路结论 {name, reason, matched}")
    check: dict | None = Field(
        default=None,
        description="day069 新增：接地核对报告（三组编号 / 覆盖率 / 接地）；空检索时为 None",
    )
    fallback_reason: str = Field(
        default="",
        description="day069 新增：这次答案为什么不是'模型给的可核对答案'；空串 = 没有回退",
    )
...
#
# 与上面五个端点同一纪律（依赖走 app.state、响应体全部可直接 json.dumps、
# 请求体只描述"怎么问"），但有三条本组独有的约定：
#
# 1. **融合参数挂在 ``RetrievalQuery.extra`` 上**（day066 为它预留的那个槽）：
#    ``strategy`` / ``alpha`` / ``k_rrf`` 是"这一次怎么合"，与"怎么问"并列。
#    继承 ``RetrievalSearchRequest`` 而不是重写一遍八个字段——它们是同一次检索的
#    两种问法，字段一旦分家，"同一个请求体在 search 与 hybrid 上校验不同"就会发生。
# 2. **缺省关闭**：``retrieval_hybrid_enabled=False`` 时 ``/retrieval/hybrid``
#    返回 400 并指出打开它的那一个开关（**不静默退回单路**——那会让
#    "我明明启用了却看不出区别"变成一个谜）。
# 3. **两路的证据原样出门**：``channel_candidates`` 与 ``fusion`` 直接来自
#    ``RetrievalResult.to_dict()``，端点不重新解释它们——
#    重新解释一次就等于把那份交代改写了一遍。
# --------------------------------------------------------------------------- #


class HybridRetrievalRequest(RetrievalSearchRequest):
    """POST /retrieval/hybrid 的请求体（M6-D6）：同一组检索参数 + 三个融合参数.

    八个检索字段与 ``/retrieval/search`` **逐字相同**（继承而来，含默认值），
    多出来的三个字段都可以省略：省略时用服务端装配的默认值
    （``HybridRetriever(strategy=..., alpha=..., k_rrf=...)``，它们又缺省来自
    ``settings.retrieval_hybrid_*``）。

    **没有 ``weights`` 字段**：权重表是一对通道名到系数的映射，而 HTTP 请求体里
    传一个"通道名 → 系数"的字典，等价于把服务端的通道命名暴露成协议的一部分。
    要按通道调权请用 ``alpha``（它就是"向量通道的权重"，关键词通道是 ``1 - alpha``）；
    需要更细的权重请注入一个自己装配好的 ``HybridRetriever``。

    一条必须写下来的**边界**：请求体里多余的 JSON 字段会被 pydantic 忽略
    （与其余端点一致），因此**拼错的字段名不会报错**——``{"weight": 0.9}``
    会得到一个 200 和一份"参数没变"的融合摘要。这与库里那条
    "``RetrievalQuery.extra`` 里写错键名报 ``FusionError``"的纪律**不是同一条通道**：
    ``extra`` 是库内 API（脚本与评估直接用），字段名错了会当场响；
    HTTP 这一层只有三个具名字段，而"多给了字段"在协议层是合法的前向兼容写法。
    因此在这里逐次覆盖请**只用** ``strategy`` / ``alpha`` / ``k_rrf`` 这三个名字。
    """

    strategy: str | None = Field(
        default=None,
        description="融合策略：rrf（只看名次）/ weighted（先归一化再加权）；None 用服务端默认",
    )
    alpha: float | None = Field(
        default=None,
        description="weighted 下**向量通道**的权重（关键词通道是 1 - alpha）；None 用服务端默认",
    )
    k_rrf: int | None = Field(
        default=None, description="RRF 的平滑常数 k；None 用服务端默认（60）"
    )


class HybridRetrievalResponse(BaseModel):
    """POST /retrieval/hybrid 的响应体（M6-D6）：融合结果 + 两路各自的明细.

    与 ``/retrieval/search`` 的差别是"多带了两路的证据"，因此多了四块：

    ```text
    channel_candidates  每路各自候选多少（{"vector": 10, "bm25": 1}）——
                        "有一路是不是根本没捞到东西"看它，单看并集看不出来
    fusion              策略 / 参数 / 每路召回条数 / 每路贡献数 / 融合/去重条数——
                        "为什么这条排第一"的全部输入
    lexical             关键词索引的现状（篇数 / 词表 / avgdl / k1 / b）——
                        "关键词路凭什么空着"的第二种回答
    channels            本次结果里出现过的召回通道
    ```

    ``result.hits[].channels`` 是**逐条**的多路证据（这一条被哪几路召回，
    按名次好坏排序）；``result.hits[].channel`` 是贡献最大的那一路。
    两者都来自 ``RetrievalResult.to_dict()``，端点不加工。

    ``lines`` 是 ``HybridRetriever.explain()``：它比 ``/retrieval/explain``
    多四行（两路明细 / 融合参数 / 阈值口径 / 关键词索引现状），
    因为这四件事只有混合检索这一层答得出来。
    """

    summary: str = Field(default="", description="一行结论")
    result: dict = Field(default_factory=dict, description="RetrievalResult 的完整投影（含正文）")
    lines: list[str] = Field(default_factory=list, description="逐行人话诊断（含混合模式四行）")
    lexical: dict = Field(default_factory=dict, description="关键词索引现状（LexicalIndex.describe()）")
    channel_candidates: dict = Field(
        default_factory=dict, description="每路各自的候选数"
    )
    fusion: dict = Field(default_factory=dict, description="融合摘要（策略 / 参数 / 召回 / 贡献）")
    empty_reason: str = Field(default="", description="空结果的唯一原因；有命中时是 hits")
    empty_reason_description: str = Field(default="", description="该原因的人话解释")
    conditions: str = Field(default="", description="过滤条件的一行可读描述")
    filter_fields: list[str] = Field(default_factory=list, description="where 里出现过的字段名")
    channels: list[str] = Field(default_factory=list, description="本次结果里出现过的通道")
    limitations: list[str] = Field(default_factory=list, description="本层的已知限制（含依据）")
    out_of_scope: list[str] = Field(default_factory=list, description="明确排除在范围外的能力")


class LexicalStatusResponse(BaseModel):
    """GET /retrieval/lexical/status 的响应体（M6-D6）：关键词索引的现状.

    四块内容，回答四个不同的问题（与 ``/vectorstore/stats`` 的三块同源）：

    ```text
    index            这份索引被建成什么样（篇数 / 词表 / avgdl / 总词元数 / k1 / b / name）
    defaults         settings 里 retrieval_bm25_* 与 retrieval_hybrid_* 的当前取值
    limitations      这一层的已知限制（含"2-gram 代替分词器"这条真实代价）
    out_of_scope     明确排除在范围外的能力
    ```

    ``count`` / ``vocabulary_size`` / ``avgdl`` / ``k1`` / ``b`` 在顶层再列一次：
    "这个库的关键词索引建好了吗、用了哪组参数"是最高频的一个问题，
    不该要求调用方在嵌套字典里找它（与 ``/retrieval/status`` 的 ``index`` 同一条理由）。

    **词表大小是一个诊断数字，不是装饰**：关键词路一条都没召回时，
    "查询词元与词表交集为空"要有一个分母（词表里到底有多少个词元），
    而没有分母的"没找到"是无法与"索引是空的"区分的。
    """

    summary: str = Field(default="", description="一行结论")
    index: dict = Field(default_factory=dict, description="LexicalIndex.describe() 的投影")
    count: int = Field(default=0, description="索引里的文档篇数")
    vocabulary_size: int = Field(default=0, description="词表大小（出现过多少个不同词元）")
    avgdl: float = Field(default=0.0, description="文档平均词元数（BM25 的归一化基准）")
    k1: float = Field(default=0.0, description="词频饱和参数（缺省 1.5）")
    b: float = Field(default=0.0, description="长度归一化强度（缺省 0.75）")
    defaults: dict = Field(default_factory=dict, description="settings 的 BM25 / 混合检索取值")
    limitations: list[str] = Field(default_factory=list, description="本层的已知限制（含依据）")
    out_of_scope: list[str] = Field(default_factory=list, description="明确排除在范围外的能力")
...
#
# 与上面七个端点同一纪律（依赖走 app.state、响应体全部可直接 json.dumps、
# 请求体只描述"怎么问 / 怎么排"），但有四条本组独有的约定：
#
# 1. **重排参数与检索参数并列，而检索那一组是继承来的**：``RerankRequest``
#    继承 ``RetrievalSearchRequest``（照 ``HybridRetrievalRequest`` 的先例），
#    因此"同一个请求体在 search 与 rerank 上校验不同"这件事不会发生——
#    八个检索字段只有一处定义。
# 2. **``min_score`` 在本组换了一个口径**：这里比的是**重排分**
#    （教学替身落在 ``[0, 1]``），而父类那一个比的是第一阶段（向量 / 融合）
#    的阈值。同名不同物是这一组最容易读错的一处，因此它在字段说明里被点名、
#    并在两个端点的 docstring 里各重复一次。
# 3. **"前后两个次序"是响应体的一等公民**：``RerankResponse`` 同时给出
#    ``before_ids`` 与 ``after_ids``——只给重排后的名单，读的人无法判断
#    "重排到底动了什么"（"名次一条都没变"与"名单本来就是这样"长得完全一样）。
# 4. **本组只有一个"强制开启"的口子**：``POST /retrieval/rerank`` 无论如何都重排
#    （它存在的唯一理由就是让人看见次序被改动了），因此 ``enabled=False``
#    是一个**明确被拒**的请求（400），而不是一个被静默忽略的字段。
# --------------------------------------------------------------------------- #


class RerankRequest(RetrievalSearchRequest):
    """POST /retrieval/rerank 的请求体（M6-D7）：同一组检索参数 + 六个重排参数.

    八个检索字段与 ``/retrieval/search`` **逐字相同**（继承而来，含默认值），
    多出来的六个字段全部可省略；``None`` 的含义逐条写在各字段说明里
    （一律是"取 settings"或"取检索层的默认"）。

    **``min_score`` 在本模型里换了口径**（本组最容易读错的一处）：父类的
    ``min_score`` 是第一阶段（向量 / 融合）的相似度阈值，而这里的 ``min_score``
    是**重排阈值**——它比的是重排分。两者同名不同物，因此本端点只提供后者：
    重排阈值恰好落在"教学替身的分数有绝对标度"这条性质上（见 ``rerank`` 模块
    docstring），而第一阶段的阈值在本请求体里不再可设（要设它请用
    ``/retrieval/search``）。写成两个名字（``min_score`` 与 ``rerank_min_score``）
    会让"我设了阈值"这句话有两个可能的所指，而它们的处置动作完全不同
    （去调召回 vs 去调重排结果）。

    **``enabled`` 的语义是本组唯一一处"拒绝"而不是"照着做"**：

    ```text
    None / True   照做（本端点无论如何都强制开启重排）
    False         → 400，消息说明"这条通道强制开启重排"与关掉它的正确位置
    ```

    ``retrieval_rerank_enabled`` 管的是**检索链路上的缺省装配**
    （``/retrieval/search`` 要不要长出重排这一步），而本端点是一条
    **显式的"让我看看重排"通道**：静默忽略 ``enabled=False`` 会让
    "我明明关了重排却看到次序变了"变成一个查不出病因的现象。
    """

    enabled: bool | None = Field(
        default=None,
        description="重排开关：None = 不表态（settings.retrieval_rerank_enabled 只进回显）；"
        "True = 照做；False = 明确要求关掉重排 → 400（本端点强制开启重排）",
    )
    mode: str | None = Field(
        default=None,
        description="重排模式：replace（窗口内只按重排分）或 blend（两份分数各自归一化后加权）；"
        "None = 取 settings.retrieval_rerank_mode（缺省 replace）",
    )
    top_n: int | None = Field(
        default=None,
        description="窗口大小：只对前 N 条打分（必须 >= 1）；"
        "None = 取 settings.retrieval_rerank_top_n（缺省 20）",
    )
    weight: float | None = Field(
        default=None,
        description="blend 下**重排分**的权重（第一阶段分数是 1 - weight），必须落在 [0, 1]；"
        "None = 取 settings.retrieval_rerank_weight（缺省 0.5，replace 下不参与排序）",
    )
    min_score: float | None = Field(
        default=None,
        description="**重排阈值**（比的是重排分，不是第一阶段的分数，只作用于被打分过的条）；"
        "None = 取 settings.retrieval_rerank_min_score（缺省不设阈值）",
    )
    model: str | None = Field(
        default=None,
        description="进报告的模型名（本层不按名字加载模型，与实现名不符时只记一条注记）；"
        "None = 取重排器自己的名字（reranker.name）",
    )


class RerankResponse(BaseModel):
    """POST /retrieval/rerank 的响应体（M6-D7）：**次序被改动了多少**的完整交代.

    ``before_ids`` 与 ``after_ids`` 是本端点存在的理由：**同一个库、同一句话、
    同一批候选**，只是最后那一步的排序规则换了。两个名单并排摆出来，
    "重排到底动了什么"就不再是一句"生效了"——而 ``rerank.moved`` 与
    ``result.hits[].stage1_rank`` 进一步回答"谁动了、从第几名到第几名"。

    ``result`` 是**写回重排结果之后**的 ``RetrievalResult.to_dict(include_text=True)``：
    命中上的 ``rerank_score``（重排分）与 ``stage1_rank``（原来的名次）已回填，
    而 ``score`` 仍然是第一阶段的分数（口径不变，见 ``RetrievalHit`` 的字段说明），
    因此"它原来得了多少分"与"重排给了它多少分"各有一个能查的地方。
    ``dropped_by_rerank`` 是**第四个数**：被重排阈值切掉几条（它不许并进
    ``dropped_by_top_k``，理由与前三道减法一样）。

    ``rerank`` 是 ``RerankResult.to_dict()``：窗口的账（``candidates`` / ``scored`` /
    ``window`` / ``top_n``）、模式与权重、逐条的四个特征、以及**注记**
    （窗口纪律那条是无条件的——"有一批条连分都没拿到"必须被看见）。

    ``lines`` 是 ``RerankResult.explain()``：窗口策略 / 模式与量纲 / 名次变化 /
    阈值落刀四段，与"读一份重排诊断"要找的四句话一一对应。
    """

    summary: str = Field(default="", description="一行结论")
    result: dict = Field(
        default_factory=dict, description="重排后的 RetrievalResult 投影（含正文与两份分数）"
    )
    rerank: dict = Field(
        default_factory=dict, description="RerankResult 的完整投影（窗口 / 模式 / 特征 / 注记）"
    )
    before_ids: list[str] = Field(
        default_factory=list, description="重排前的 id 次序（第一阶段给的那份名单）"
    )
    after_ids: list[str] = Field(
        default_factory=list, description="重排后的 id 次序（本端点要展示的那份名单）"
    )
    lines: list[str] = Field(default_factory=list, description="逐行人话诊断（窗口 / 模式 / 名次）")
    route: dict = Field(default_factory=dict, description="选路结论 {name, reason, matched}")
    empty_reason: str = Field(default="", description="空结果的唯一原因；有命中时是 hits")
    empty_reason_description: str = Field(default="", description="该原因的人话解释")
    defaults: dict = Field(default_factory=dict, description="settings 的 retrieval_rerank_* 取值")
    limitations: list[str] = Field(default_factory=list, description="本层的已知限制（含依据）")
    out_of_scope: list[str] = Field(default_factory=list, description="明确排除在范围外的能力")


class RerankStatusResponse(BaseModel):
    """GET /retrieval/rerank/status 的响应体（M6-D7）：重排配置 + 模型自述 + 缺省值表.

    四块内容**不混在一起**，理由与 ``/retrieval/status`` 的三块同源：

    ```text
    model      这次会用来打分的那个重排器的自述（CrossEncoderReranker.describe()）：
               名字 / 类型（带 (teaching) 标记）/ 四个特征 / 权重 / 批大小 + 三个计数器
    enabled    检索链路上重排开没开（settings.retrieval_rerank_enabled）
    top_n/mode/weight/min_score  缺省装配会用的那组重排参数（settings.retrieval_rerank_*）
    note       "教学级替身"的诚实声明（一句话，见 RERANK_TEACHING_NOTE）
    ```

    **为什么那四个参数读的是 settings 而不是注入的重排器**：它们不属于重排器
    （``top_n`` 是窗口、``mode``/``weight`` 是混合方式、``min_score`` 是阈值），
    而属于**检索链路上的装配**（``Retriever(rerank_top_n=...)`` 那一组）。
    注入一个重排器只改变"谁打分"，不改变这四个数——把它们混在一起读，
    会得出"换了个重排器，窗口也变了"这种不存在的因果。

    **三个计数器在缺省装配下永远是 0**：重排器是**每次请求现装**的
    （与 ``retrieval_router`` 同一条纪律：不缓存，否则"注入的重排器被换掉"
    会在某个请求上悄悄失效）。要看累计账请注入一个实例挂在
    ``app.state.retrieval_rerank`` 上——那时 ``calls`` / ``scored_pairs`` /
    ``batches`` 才是"这台机器一共打了多少次分"，而 ``scored_pairs`` 正是
    **窗口纪律的账**（它应该恰好等于每次重排的 ``scored`` 之和）。
    """

    summary: str = Field(default="", description="一行结论（重排器自述）")
    model: dict = Field(
        default_factory=dict, description="重排器自述（名字 / 类型 / 特征 / 权重 / 计数器）"
    )
    enabled: bool = Field(default=False, description="检索链路上是否启用重排（settings）")
    top_n: int = Field(default=0, description="窗口大小（settings.retrieval_rerank_top_n）")
    mode: str = Field(default="", description="重排模式（settings.retrieval_rerank_mode）")
    weight: float = Field(default=0.0, description="blend 下的重排分权重（settings）")
    min_score: float | None = Field(
        default=None, description="重排阈值（settings；None 表示不设阈值）"
    )
    note: str = Field(default="", description="教学级替身的诚实声明（它没有真实语义）")
    defaults: dict = Field(default_factory=dict, description="settings 的 retrieval_rerank_* 取值")
    limitations: list[str] = Field(default_factory=list, description="本层的已知限制（含依据）")
    out_of_scope: list[str] = Field(default_factory=list, description="明确排除在范围外的能力")
...
class RerankCompareRequest(BaseModel):
    """POST /retrieval/rerank/compare 的请求体（M6-D7）：一句话 + 它的金标准 id.

    **它的输入是一"条"查询而不是一"批"探针**（与 ``rerank.measure_lift``
    的 ``Sequence[LiftProbe]`` 不同）：服务端自己跑"检索 → 重排 → 算指标"，
    因此 ``per_query`` 的长度恒为 1。字段形状保留是为了与 ``measure_lift``
    的逐条明细**逐字对齐**（同一个键名在脚本与接口上指的是同一件事）。

    ```text
    query      那句话（重排分是对 (查询, 文档) 打的，因此查询必须跟着走）
    relevant   金标准 id：**不能为空**（三条指标的分母都由它给出，
               空标注会让 recall@k 变成 0/0，而它算出来的 0.0 看起来像
               "一条都没召回到"）
    k          三条指标共用的那个 k（recall@k / nDCG@k），必须 >= 1
    top_n / mode / weight  与 /retrieval/rerank 同义的三个重排参数（都可省略）
    ```

    **没有 ``min_score``**：对照报告问的是"换排序有没有用"，而重排阈值是
    "只留下分数够高的那几条"（它改变的是名单长度，不是排序质量），
    两者混在一张前后对照表里会让"提升"这个词同时指两件事。
    也没有 ``route``：它测的是**这台机器的缺省检索路径**；多索引且没设默认路由时
    本端点会 400 并列出可用名（与 ``StoreRouter.resolve`` 同一条规则）。
    """

    query: str = Field(default="", description="查询文本（空串或空白串 → 400）")
    relevant: list[str] = Field(
        default_factory=list, description="金标准记录 id（不能为空；空列表 → 400）"
    )
    k: int = Field(default=5, description="三条指标共用的 k（必须 >= 1，越界 → 400）")
    top_n: int | None = Field(
        default=None, description="窗口大小；None = 取重排层的缺省（20）"
    )
    mode: str | None = Field(
        default=None, description="重排模式；None = 取重排层的缺省（replace），未知取值 → 400"
    )
    weight: float | None = Field(
        default=None, description="blend 下的重排分权重；None = 取重排层的缺省（0.5）"
    )


class RerankCompareResponse(BaseModel):
    """POST /retrieval/rerank/compare 的响应体（M6-D7）：**重排有没有用**的对照表.

    三条指标（公式见 ``rerank`` 模块 docstring）各自回答一个不同的问题，
    ``before`` / ``after`` / ``lift`` 三张表**逐键对齐**：

    ```text
    recall            前 k 条里命中了金标准的几条 / 金标准条数   → 有没有捞回来
    reciprocal_rank   1 / (第一个相关命中的位置 + 1)             → 第一条要多久才相关
    ndcg              命中几条 + 排得多靠前一起看（二值增益）      → 唯一同时看两者的那条
    lift              after - before（**正数 = 重排在这条指标上更好**；用差值而不是
                      比值：before == 0 时比值是无穷）
    ```

    ``per_query`` 长度恒为 1（本端点的输入是一条查询），里面是与
    ``measure_lift`` **逐字相同**的明细：两侧的名单、三条指标、谁的名次动了、
    窗口的账。它保留成列表而不是摊平成顶层字段，是为了让"接口的这一次对照"
    与"脚本跑一批探针的那份报告"能放进同一张表里读。

    ``before`` 与 ``after`` 用的是**同一批命中**（服务端只检索一次，
    重排作用在同一份名单上）：因此两张表之差里**只混着一个变量**
    ——"有没有重排"，而不是"检索了两次"。
    """

    summary: str = Field(default="", description="一行结论（三条指标的前后与差值）")
    before: dict = Field(default_factory=dict, description="重排前的三条指标")
    after: dict = Field(default_factory=dict, description="重排后的三条指标")
    lift: dict = Field(default_factory=dict, description="after - before（正数 = 重排更好）")
    per_query: list[dict] = Field(
        default_factory=list, description="逐条明细（长度恒为 1：两侧名单 / 指标 / 移动 / 窗口账）"
    )
    metrics: list[str] = Field(
        default_factory=list, description="指标名（字典序，与三张表的键一一对应）"
    )
    queries: int = Field(default=0, description="评了几条查询（本端点恒为 1）")
    k: int = Field(default=0, description="三条指标共用的那个 k")
    defaults: dict = Field(default_factory=dict, description="settings 的 retrieval_rerank_* 取值")
    limitations: list[str] = Field(default_factory=list, description="本层的已知限制（含依据）")
    out_of_scope: list[str] = Field(default_factory=list, description="明确排除在范围外的能力")
...
#
# 与上面十条端点同一纪律（依赖走 app.state、响应体全部可直接 json.dumps、
# 请求体只描述"怎么问 / 怎么答"），但有四条本组独有的约定：
#
# 1. **生成参数与检索参数并列，而检索那一组是继承来的**：``GenerateRequest``
#    继承 ``RetrievalSearchRequest``（照 ``RerankRequest`` 的先例），因此八个
#    检索字段只有一处定义——"同一个请求体在 search 与 generate 上校验不同"
#    这件事不会发生。文本字段因此叫 ``query``（不是 ``question``）：
#    它就是被编码的那句话，进提示词之后才成为 ``Generation.question``。
# 2. **提示词只能用版本号说，不能在请求体里传模板文本**：``prompt_version``
#    只接受**受管版本名**（``v1`` / ``v2``，清单见 ``generation.PROMPT_VERSIONS``），
#    不接受模板文本、也不接受清单外的版本号——原因见 ``generation`` 的模块
#    docstring：模板一旦能被请求方改写，``prompt_version`` 就不再指着一份确定的
#    模板，"换一版提示词之后可核对率变了吗"（day071）也随之失去可复现性。
# 3. **``fallback_reason`` 与 ``llm_called`` 分开出现在响应里**：前者回答
#    "这次答案为什么不能按正常路径读"，后者回答"模型有没有机会自由发挥"
#    （护栏的证据）。合成一个字段之后，"调了但答案没引用"与"根本没调"
#    会变成同一个值——而这两件事的处置动作完全不同。
# 4. **核对可以单独问一次**（``/retrieval/grounding/verify``）：它**一次模型都不调**，
#    只拿一份编号表与一段答案对账，因此"有没有幻觉引用"这个结论能在不花钱、
#    不联网的前提下被复核（与生成链路用的是同一份核对实现）。
# --------------------------------------------------------------------------- #


class GenerateRequest(RetrievalSearchRequest):
    """POST /retrieval/generate 的请求体（M6-D8）：同一组检索参数 + 四个生成参数.

    八个检索字段与 ``/retrieval/search`` **逐字相同**（继承而来，含默认值），
    多出来的四个字段全部可省略；``None`` 一律表示"取 settings"（见各字段说明）。
    反过来读，这四个字段就是"这一次生成可以在不改库的前提下换掉哪几件事"：
    换一版提示词、换采样温度、换长度上限、换"没有引用的答案算不算数"。

    三处刻意的不提供：

    ```text
    没有 prompt / template 字段  提示词**只能**用版本号说：模板是"这一次模型到底
                                被怎么问的"的唯一证据，一旦它能被请求方改写，
                                prompt_version 就不再指着一份确定的模板
    （同样地）清单外的版本号      一律 400 并列出合法取值，不"没见过就当默认"：
                                版本号是报告与评估的分组键
    没有 max_context_chars /     打包预算是**装配**的事（settings.retrieval_max_context_chars
    per_hit_chars                / retrieval_per_hit_chars），不随一次请求变——
                                要按请求调预算请用 /retrieval/answer 那条链路
    ```

    文本字段是继承来的 ``query``（不是 ``question``）：与 ``/retrieval/search``、
    ``/retrieval/rerank`` 同一处定义，因此同一个请求体在这三条通道上校验一致；
    它进提示词之后就是 ``Generation.question``。
    """

    prompt_version: str | None = Field(
        default=None,
        description="提示词版本号：**只接受受管版本名**（v1 / v2），"
        "不允许直接传模板文本；None = 取 settings.retrieval_prompt_version（缺省 v2）",
    )
    temperature: float | None = Field(
        default=None,
        description="采样温度（必须落在 [0, 2]）：越高模型越会改写片段里的事实；"
        "None = 取 settings.retrieval_answer_temperature（缺省 0.0）",
    )
    max_tokens: int | None = Field(
        default=None,
        description="一次回答的长度上限（必须 >= 1）：给 0 会换来一次空回复；"
        "None = 取 settings.retrieval_answer_max_tokens（缺省 1024）",
    )
    require_citation: bool | None = Field(
        default=None,
        description="是否把'一条合法引用都没有'当作不可交付（答案改取兜底答复）；"
        "None = 取 settings.retrieval_require_citation（缺省 False）",
    )


class GenerateResponse(BaseModel):
    """POST /retrieval/generate 的响应体（M6-D8）：答案 + 怎么问的 + 核对结果.

    三块内容**不混在一起**（与 ``/retrieval/rerank`` 的三块同源）：

    ```text
    答案这一块   answer / llm_called / fallback_reason(_description) / notes
                 ——"这次到底发生了什么"，其中 llm_called 是护栏是否生效的证据
    问法这一块   prompt_version / prompt_text / context / citations
                 ——"模型看到了什么"，prompt_text 是排查一次错误答案的第一份证据
    核对这一块   grounding（三个编号集合 + 覆盖率 + 接地）与 check（逐编号的检查行）
                 ——"答案里的 [n] 指得到谁"，两者是同一份报告的整体与明细
    ```

    ``grounding`` 与 ``check`` **刻意都出现**：前者是"给了几条 / 引用几条 / 幻觉几条 /
    覆盖多少 / 接地通过没有"的账，后者是逐编号的行（哪一条是幻觉引用、哪几条给了没用上）。
    只给前者时"具体是哪几个编号"要再解析一次嵌套字典；只给后者时读的人要自己数。

    ``context`` 不是 ``None``（与 ``/retrieval/answer`` 不同）：这一条链路即使检索为空
    也照样走"打包 → 生成"，因此打包结果是一份**空包**（``count=0``、
    ``text=""``、``citations=[]``）——"这次打包了什么"在空检索时同样可回答。

    ``retrieval`` 是那次检索的完整交代（含 ``empty_reason`` 与索引版本 / 漂移），
    它让"答案不对"这件事能往上游追一层：是检索给错了片段，还是提示词没把片段喂进去。
    """

    summary: str = Field(default="", description="一行结论（问 / 答 / 提示词版本 / 核对 / 回退）")
    answer: str = Field(default="", description="交付给用户的答案；回退时是兜底答复或空串")
    llm_called: bool = Field(default=False, description="这次到底调没调模型（护栏是否生效的证据）")
    prompt_version: str = Field(default="", description="这次真正用的是哪一版提示词（分组键）")
    prompt_text: str = Field(default="", description="真正发出去的那段提示词（模型这次看到了什么）")
    citations: list[dict] = Field(
        default_factory=list, description="那次提示词里的编号表（编号 → 记录 id / 来源 / 分数）"
    )
    grounding: dict = Field(
        default_factory=dict,
        description="GroundingReport 的投影（三组编号 + 覆盖率 + 接地 + 注记）",
    )
    fallback_reason: str = Field(
        default="", description="六取一：为什么这次答案不能按正常路径读；空串 = 没有回退"
    )
    fallback_reason_description: str = Field(default="", description="该原因的人话解释与出路")
    notes: list[str] = Field(
        default_factory=list, description="回退的出路 / 幻觉引用 / 未引用条数 / 被忽略的参数"
    )
    context: dict | None = Field(
        default=None, description="进提示词的那段文本及其账（检索为空时是一份空包，不是 None）"
    )
    retrieval: dict | None = Field(default=None, description="那次检索的完整交代")
    empty_reason: str = Field(default="", description="那次检索的空结果原因（有命中时是 hits）")
    route: dict = Field(default_factory=dict, description="选路结论 {name, reason, matched}")
    check: list[dict] = Field(
        default_factory=list,
        description="逐编号的检查行（先 1..n，再按升序接上越界编号；record_id 为空即幻觉引用）",
    )
    defaults: dict = Field(default_factory=dict, description="settings 的 retrieval_* 生成组取值")
    limitations: list[str] = Field(default_factory=list, description="本层的已知限制（含依据）")
    out_of_scope: list[str] = Field(default_factory=list, description="明确排除在范围外的能力")


# --------------------------------------------------------------------------- #
# day071 RAG 评估（M6-D9）：/rag/eval/* 两个端点
# --------------------------------------------------------------------------- #


class RagEvalStatusResponse(BaseModel):
    """GET /rag/eval/status 的响应体（M6-D9）：套件现状 + 指标口径 + 变体表 + 基线.

    五块内容各自回答一个问题，**不混在一起**（与 ``/retrieval/status`` 同一取向）：

```text
suite      这份套件被装配成什么样（评测集几条、语料几条、k、判据两个阈值）
variants   四个变体各自"只改了哪一项"（一次只改一个旋钮）
metrics    11 个质量指标的名字 / 方向 / 说明——**口径本身也要能被读出来**
tags       12 类坏例标签的解释与处置动作（报告里那张表的完整版）
baseline   已提交的那份质量基线（没有时是 None，而不是一份全零的假基线）
```

    ``metrics`` 与 ``tags`` 是这一课最有用的两块：报告里的数字只有配上
    "它往哪个方向算好"才可读（``higher_better`` / ``lower_better`` /
    ``neutral``），而坏例标签只有配上"下一步做什么"才可动手。
    """

    summary: str = Field(default="", description="一行结论（套件规模与判据）")
    suite: dict = Field(default_factory=dict, description="RagEvalSuite.describe() 的投影")
    defaults: dict = Field(default_factory=dict, description="settings 的 rag_eval_* 当前取值")
    variants: list[dict] = Field(
        default_factory=list, description="变体表（name / 改了什么 / note）"
    )
    metrics: list[dict] = Field(
        default_factory=list, description="11 个指标的 {metric, direction, description}"
    )
    tags: dict = Field(default_factory=dict, description="12 类坏例标签的 {description, action}")
    baseline: dict | None = Field(
        default=None, description="已提交的质量基线（不存在时为 null，不是全零基线）"
    )
    limitations: list[str] = Field(default_factory=list, description="本次结论的适用范围")
    out_of_scope: list[str] = Field(default_factory=list, description="明确排除在范围外的能力")


class RagEvalRunRequest(BaseModel):
    """POST /rag/eval/run 的请求体（M6-D9）：跑哪个变体、评不评忠实度、比不比基线."""

    variant: str | None = Field(
        default=None,
        description="要跑的变体名（见 /rag/eval/status 的 variants）；不传 = 表里的第一个",
    )
    judge: bool = Field(
        default=False,
        description="是否用评审模型评忠实度（打开后每条用例多一次模型调用）",
    )
    compare_with_baseline: bool = Field(
        default=True,
        description="若基线文件存在，是否顺带做一次质量对比（不下结论时如实写出来）",
    )


class RagEvalRunResponse(BaseModel):
    """POST /rag/eval/run 的响应体（M6-D9）：一份完整报告 + 折出的基线 + 对比结论.

    三块内容是**同一批用例的三种读法**：

```text
report      全套证据（11 个指标 + 逐条用例 + 坏例表 + 动作清单 + 适用范围）
baseline    折出来的那 11 个数字（它就是可以进 git、可以 diff 的那一份）
comparison  与已提交基线的对比（基线不存在时为 null；不可比时 conclusive=false）
```

    ``report.per_case`` 是逐条证据（每条用例一行）；要比对性能问题请用它，
    要看结论请读 ``report.metrics``，要动手请读 ``report.actions``——
    这三种读者的需求被刻意分开成三个字段，而不是塞进一个大字典。
    """

    report: dict = Field(default_factory=dict, description="RagEvalReport.to_dict() 的投影")
    baseline: dict = Field(default_factory=dict, description="报告折出的基线（11 个指标）")
    comparison: dict | None = Field(
        default=None, description="与已提交基线的对比；null = 没有基线或未要求对比"
    )
    judge: bool = Field(default=False, description="本次是否评了忠实度（回显，便于复现）")
    limitations: list[str] = Field(default_factory=list, description="本次结论的适用范围")
    out_of_scope: list[str] = Field(default_factory=list, description="明确排除在范围外的能力")


class PromptVersionsResponse(BaseModel):
    """GET /retrieval/prompt/versions 的响应体（M6-D8）：受管提示词的清单与元信息.

    它是一条**纯读端点**的返回体：不检索、不调模型、不写任何状态。
    五块内容各自回答一个不同的问题：

    ```text
    versions / current / default  有几版受管模板、现在生效的是哪一版、没指定时用哪一版
    templates                     逐版：字符数 / 变更原因 / 占位符 / 约束条数——
                                  "换一版"到底换了什么，靠这四栏读
    fallback_reasons              六种回退的**全部**取值与人话解释（含表示"没有回退"的空串）
    decline_markers               固定拒答句式（检测清单与 v2 的约束 2 是同一份）
    defaults                      settings 里那四个生成旋钮的当前取值
    ```

    ``default`` 与 ``current`` 今天相同（都是 ``v2``）但**服务两个问题**：
    前者回答"没指定时用哪一版"（``settings.retrieval_prompt_version``），
    后者回答"现在生效的模板是哪一份"（``generation.CURRENT_PROMPT_VERSION``）。
    换版本时只改后者；改前者是一次会让所有调用方一起换版本的显式决定。
    """

    summary: str = Field(default="", description="一行结论（几版 / 当前是哪一版 / 拒答句式几条）")
    versions: list[str] = Field(
        default_factory=list, description="受管版本号（顺序 = 报告里的顺序）"
    )
    current: str = Field(default="", description="当前生效的模板版本号")
    default: str = Field(default="", description="没指定版本号时用的那一版")
    templates: list[dict] = Field(
        default_factory=list,
        description="逐版元信息：version / chars / changelog / placeholders / constraints",
    )
    fallback_reasons: list[dict] = Field(
        default_factory=list, description="回退原因的全部取值与人话解释（含空串 = 没有回退）"
    )
    decline_markers: list[str] = Field(
        default_factory=list, description="固定拒答句式（v2 要求模型用的那三句）"
    )
    defaults: dict = Field(default_factory=dict, description="settings 的 retrieval_* 生成组取值")
    limitations: list[str] = Field(default_factory=list, description="本层的已知限制（含依据）")
    out_of_scope: list[str] = Field(default_factory=list, description="明确排除在范围外的能力")
...
class GroundingVerifyCitation(BaseModel):
    """离线核对请求里那份编号表的一行：``[n]`` 指到哪一条记录（M6-D8）.

    字段与 ``retrieval.context.Citation`` **逐项对应**，只把三个"给人看"的字段
    变成可省略（``source`` / ``heading_path`` / ``score``）：核对只读编号与记录 id，
    而"这句话出自哪里"与分数都不参与对账（见 ``GroundingReport`` 的核对口径）。
    """

    marker: int = Field(description="编号，必须是 >= 1 的整数（0 不是编号，见 Citation.marker）")
    record_id: str = Field(description="这条编号指向的记录 id（不能是空串）")
    source: str = Field(default="", description="给人看的来源（可省略；不参与核对）")
    heading_path: str = Field(default="", description="标题路径（可省略；不参与核对）")
    score: float = Field(default=0.0, description="这一条的分数（可省略；不参与核对）")


class GroundingVerifyRequest(BaseModel):
    """POST /retrieval/grounding/verify 的请求体（M6-D8）：一份编号表 + 一段答案.

    两个字段都**必填**（离线核对没有"默认答案"可言），但 ``citations`` 可以是
    空列表——"这次没有片段可核对"是一份合法的核对结果（它会带回一条注记），
    而不是一个错误。

    为什么给的是**编号表**而不是一段带 ``[1] …`` 的资料文本：从文本里解析出编号
    要再定一套"编号写在哪儿、正文到哪儿结束"的语法，而那一套与 ``pack_context``
    的渲染规则一定会分家（分家的表现是"端点数出来的编号与生成链路的不一致"）。
    给编号表就没有这个问题：它就是提示词里那份唯一的编号来源。
    """

    answer: str = Field(
        description="要核对的那段答案（里面形如 [1] / [ 2 ] 的编号会被逐个对账）"
    )
    citations: list[GroundingVerifyCitation] = Field(
        description="那次提示词里的编号表；空列表 = 这次没有东西可引（合法状态）"
    )


class GroundingVerifyResponse(BaseModel):
    """POST /retrieval/grounding/verify 的响应体（M6-D8）：一份核对报告（**不调模型**）.

    六个数字一起才说得出"这段答案有没有依据"（与 ``GroundingReport.to_dict`` 同一份）：

    ```text
    given     那次给了几条片段（= len(valid) + len(unused)，算出来的）
    ...
    ``grounded`` 的定义写死在一处（``valid 非空 且 invalid 为空``），因此它
    **不是**一次事实核查：通过只说明"有可核对的依据"，不说明这句话忠实于片段
    （语义判断不在这一层）。``checks`` 是逐编号的明细行（哪一条是幻觉引用、
    哪几条白给了），``notes`` 是关于**这份报告本身**的说明（空引用、非编号的 [0]）。
    """

    summary: str = Field(
        default="", description="一行结论（给了几条 / 引用几条 / 覆盖多少 / 接地）"
    )
    given: int = Field(default=0, description="这次给出去的片段条数（valid + unused）")
    cited: list[int] = Field(default_factory=list, description="答案里出现过的编号（升序去重）")
    valid: list[int] = Field(default_factory=list, description="落在编号表里的编号（升序）")
    invalid: list[int] = Field(default_factory=list, description="落不到编号表里的编号 → 幻觉引用")
    unused: list[int] = Field(default_factory=list, description="给了但它没引用的编号（升序）")
    coverage: float = Field(default=0.0, description="len(valid) / given，落在 [0, 1]")
    grounded: bool = Field(default=False, description="valid 非空且 invalid 为空（定义写在一处）")
    hallucinated: int = Field(default=0, description="幻觉引用的条数（len(invalid)）")
    checks: list[dict] = Field(
        default_factory=list,
        description="逐编号的检查行（marker / record_id / hallucinated / used）",
    )
    notes: list[str] = Field(default_factory=list, description="关于这份报告本身的说明")
    defaults: dict = Field(default_factory=dict, description="settings 的 retrieval_* 生成组取值")
    limitations: list[str] = Field(default_factory=list, description="本层的已知限制（含依据）")
    out_of_scope: list[str] = Field(default_factory=list, description="明确排除在范围外的能力")
...
class RagOpsStatusResponse(BaseModel):
    """GET /rag/ops/status 的响应体（M6-D10）：运维现状 + 调度判定 + 监控 + 编排校验.

    五块内容都**只读**（本端点不跑同步、不跑评估、不写任何文件）：

    ```text
    schedule   账本（水位/上次成功/连续失败）+ 策略 + 判定（该不该跑、下一次在哪）
    index      索引现状（day065 的 stats：后端、维度、条数、版本数、备份数、缓存）
    metrics    本进程内的监控采样汇总（没量到的指标不出现，而不是填 0）
    deploy     编排规格的静态校验结论 + 与仓库里那份 yml 的一致性
    last_report 上一次落盘的报告（没有时为 null，**不造假报告**）
    ```

    ``deploy.valid`` 与 ``deploy.compose_drift`` 是这一课特意放进状态里的两块：
    "编排文件与规格不一致"这类问题在部署时才发现的代价很大，
    而它本来只是一次字符串比较。
    """

    summary: str = Field(default="", description="一行结论（调度判定 + 索引条数 + 健康）")
    source_dir: str = Field(default="", description="语料根目录")
    ledger_path: str = Field(default="", description="账本路径（空串 = 不落盘）")
    report_path: str = Field(default="", description="报告路径（空串 = 不落盘）")
    schedule: dict = Field(default_factory=dict, description="策略 + 判定 + 账本摘要")
    index: dict = Field(default_factory=dict, description="IndexingPipeline.stats() 的投影")
    metrics: list[dict] = Field(default_factory=list, description="监控采样汇总（每个指标一行）")
    deploy: dict = Field(default_factory=dict, description="编排规格 + 11 条判据 + 文件一致性")
    last_report: dict | None = Field(
        default=None, description="上一次落盘的报告（不存在时为 null，不是空报告）"
    )


class RagOpsHealthResponse(BaseModel):
    """GET /rag/ops/health 的响应体（M6-D10）：四项检查 + 三态 + 探针退出码.

    这个端点是**给容器探针用的**，因此它有一处刻意的不对称：
    ``state == "fail"`` 时 HTTP 状态码是 **503**，其余是 200。

    ```text
    curl -f http://localhost:8080/rag/ops/health
        → 200：这个实例可以继续接收流量
        → 503：curl -f 返回非零，编排层据此把它摘下来
    ```

    如果永远返回 200，探针就永远成功——那等于没有探针。而"把状态放在响应体里
    让人自己看"在自动化编排里不是一条可用的约定。
    """

    summary: str = Field(default="", description="一行结论（总状态 + 三态计数 + 退出码）")
    state: str = Field(default="ok", description="总状态：ok / warn / fail（取最坏的一条）")
    ok: bool = Field(default=True, description="能不能继续服务（fail 才算不能）")
    exit_code: int = Field(default=0, description="给容器探针与 CI 用的退出码（warn 仍为 0）")
    checked_at: str = Field(default="", description="体检时刻（秒级 UTC）")
    findings: list[dict] = Field(
        default_factory=list,
        description="四项检查的判据（check / state / message / detail）",
    )
    rules: list[str] = Field(default_factory=list, description="健康检查项清单（口径本身可读）")


class RagOpsSyncRequest(BaseModel):
    """POST /rag/ops/sync 的请求体（M6-D10）：要不要忽略调度判定."""

    force: bool = Field(
        default=False,
        description="忽略间隔与退避，立即执行一次（等价于 worker 的 --force）",
    )


class RagOpsSyncResponse(BaseModel):
    """POST /rag/ops/sync 的响应体（M6-D10）：一份完整运行报告 + 一行结论.

    ``report`` 是 ``OpsReport.to_dict()`` 的完整投影（状态 / 调度判定 / 差集 /
    构建报告 / 体检 / 采样 / notes）。它**不因为失败而变成 400**：

    ```text
    status = not_due / no_change / synced / failed    四取一，都在 report 里
    HTTP   200：本次运行有结论（哪怕是失败）——报告本身就是产物
    HTTP   400：请求本身不可执行（例：语料目录不存在、参数非法）
    ```
    ...
    summary: str = Field(default="", description="一行结论（状态 + 差集 + 健康 + 耗时）")
    status: str = Field(default="", description="not_due / no_change / synced / failed")
    report: dict = Field(default_factory=dict, description="OpsReport.to_dict() 的投影")
    notes: list[str] = Field(default_factory=list, description="本次运行的逐条说明")
    persisted: dict = Field(
        default_factory=dict, description="本次落盘了什么（账本 / 报告的路径）"
    )
