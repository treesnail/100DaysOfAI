...
class Settings(BaseSettings):
    ...
    # 检索（M6-D5）：问法的默认值——深度、阈值、每文档条数、时间字段与打包预算。
    # 这一组与 `indexing_*` 的差别是这一组最该记住的一件事：
    # **它们不改变任何产物的字节**（不改 chunk_id、不改向量、不改清单版本号），
    # 只改变"怎么问"——因此调它们随时可以生效，**不需要**重建索引。
    # 唯一的例外是 retrieval_strict_index：它不改变结果，只改变"漂移时是否拒绝服务"。
    # 默认返回条数。缺省 5（与 vector_store 的 vector_default_top_k 同量级）：
    # 5 条通常已经覆盖一个问题的主要依据，再多只会稀释模型注意力。
    retrieval_top_k: int = 5
    # 过取倍率：去重/阈值/多样性都要吃名额，所以从库里取的条数要更多。
    # 缺省 3 是"三道减法里最狠的一道（max_per_doc=1）也能凑够结果"的下界；
    # 再大只是白扫（见 retrieval/types.DEFAULT_FETCH_MULTIPLIER）。
    retrieval_fetch_multiplier: int = 3
    # 相似度下限（统一口径：越大越近）。None 表示不设阈值。
    # 不设默认值的理由与 vector_min_score 相同：它在 l2 度量下是另一句话，
    # 而余弦的绝对尺度要按语料标定——标定之前"给一个数字"是假的安全感。
    retrieval_min_score: float | None = None
    # 每个文档最多留几条。缺省 0 = 不限。
    # 不限的理由：它默认会**减少**结果（同一文档刷屏时把名额让给别的文档），
    # 而那是一种需要判断力的取舍（要不要看同一份文档的多段）。
    # 因此默认保持"取最像的 K 条"，由使用者按语料显式打开。
    retrieval_max_per_doc: int = 0
    # 时间过滤的默认字段名。取 created_at 是因为它是**显式入库的时间戳**；
    # 用文件的 mtime 会让"同一份文档在两台机器上落在不同时间区间里"。
    retrieval_time_field: str = "created_at"
    # 送进提示词的上下文预算（chars）。缺省 2400 按"6 个 ~400 字的片段"
    # 标定：够放下 top-5 的全部正文，又不至于把 4k 上下文撑满
    # （还要留系统提示、历史与回答本身）。
    retrieval_max_context_chars: int = 2400
    # 单条命中的上限（chars）。缺省 800 = 预算的 1/3：
    # 一条过长时先截断它，而不是让它把整包预算吃光——
    # 截断会记进 truncated_hits，因此"这次回答是不是被截断过"是可回答的。
    retrieval_per_hit_chars: int = 800
    # 严格模式：清单与库不一致时**拒绝服务**（而不是带着告警照常返回）。
    # 缺省 False 的理由：漂移下检索仍然能跑、只是可能少召回，
    # 而"因为一份 JSON 过期就整体不可用"是更重的代价。
    # 打开它说明"这个环境的结果必须可追溯到确定的索引版本"（例如评估与发布验证）。
    retrieval_strict_index: bool = False
    # 混合检索（M6-D6）：向量路 + 关键词路（BM25）怎么合、合用哪一组参数。
    # 这一组与上面的 retrieval_* 是**同一类**（都不改变任何产物的字节，只改变"怎么问"），
    # 只多了一条与它们不同的纪律：新能力**默认不生效**。
    # 是否在检索链路上启用混合检索。缺省 False 的理由不是保守，而是"结果集合会变"：
    ...
    # 融合策略：rrf（默认，只看名次）/ weighted（先归一再加权）。
    # 默认 rrf 的理由是**跨通道的分数量纲不可比**：余弦 ∈ [-1, 1]、BM25 ∈ [0, ∞)，
    # 直接加权会把"量纲差"当成"相关性差"（见 retrieval/fusion.py 的论证）。
    retrieval_hybrid_strategy: str = "rrf"
    # weighted 策略下向量通道的权重（关键词路的权重是 1 - alpha）。
    # 缺省 0.5 是"两边都不占先"的起点；调它之前请先用 rrf 确认两路**都有**贡献
    # ——把一路上不存在的证据当成可以靠权重调出来的东西，是这一课要拆的一个幻觉。
    retrieval_hybrid_alpha: float = 0.5
    # RRF 的平滑常数 k。缺省 60 出自 RRF 原论文（Cormack et al., 2009）：
    # 它把"第 1 名与第 2 名"的差距压到约 1/60，于是名次靠后的通道仍能贡献证据；
    # k 越小越"只认头部"，k=1 时第 1 名贡献 1/2、第 2 名 1/3，几乎只信排名第一的那条。
    retrieval_hybrid_rrf_k: int = 60
    # BM25 的 k1（词频饱和项）。缺省 1.5，出自 Robertson & Zaragoza 的经典取值
    # （"词频收益递减"的两个标准参数之一）。k1=0 退化成"完全忽略词频"。
    retrieval_bm25_k1: float = 1.5
    # BM25 的 b（长度归一化强度）。缺省 0.75，与 k1 同一处出处。
    # b=1 表示完全按文档长度归一化（长文档里的同一个词更不值钱），b=0 表示完全不归一化。
    # 0.75 的意思是"长文档打个折，但不把长度当成决定性的因素"。
    retrieval_bm25_b: float = 0.75
    # 重排序（M6-D7）：交叉编码器只对前 N 条候选重打分，把"最像"换成"最相关"。
    # 这一组与上面的 retrieval_* 是**同一类**（都不改变任何产物的字节：
    # 不改 chunk_id、不改向量、不改清单版本号，只改变"怎么问"），
    # 因此调它们随时生效、**不需要**重建索引。
    # 它也遵守 day067 定下的那条纪律：新能力**默认不生效**——
    # 重排会**改变名单顺序**（那正是它存在的意义），因此"换一层排序"必须是一次显式决定，
    # 而不是一次升级之后自动发生的事。关着的时候 day066 与 day067 的结果逐位不变。
    # 是否在检索链路上启用重排。缺省 False。
    retrieval_rerank_enabled: bool = False
    # 重排模型名。缺省是**教学级替身**（cross-encoder-teaching-v1）：
    # 它没有真实语义（四个可手算特征代替 transformer），分数只能用于相对排序。
    # 生产环境请替换为真实重排模型——接口是 rerank.BaseReranker，
    # 换实现时**不需要**改本项以外的任何配置（本课不按名字加载模型）。
    retrieval_rerank_model: str = "cross-encoder-teaching-v1"
    # 只对前 N 条打分（窗口纪律）。缺省 20 是"交叉编码器比双塔贵两个数量级"与
    # "第 21 名还值不值得救"之间的折中：窗口内每条一次前向推理，窗口外一条都不打分。
    # 调大它更准也更慢；窗口外的命中**永远没有机会**证明自己（这一点会进结果里的 notes）。
    retrieval_rerank_top_n: int = 20
    # blend 模式下**重排分**的权重（第一阶段分数的权重是 1 - weight）。
    # 缺省 0.5 是"两边都不占先"的起点：两个分数量纲不可比，因此先各自 min-max
    # 归一化再加权；调它之前请先跑一遍 replace，看清重排到底把谁提到了前面。
    retrieval_rerank_weight: float = 0.5
    # 重排模式：replace（默认，窗口内只按重排分重排）/ blend（归一化后加权）。
    # 默认 replace 的理由：多数时候第一阶段的分数只用来**召回**，
    # 而"重排之后还把它混进来"需要额外的理由（它可能就是错的那些条）。
    retrieval_rerank_mode: str = "replace"
    # 重排阈值：只作用于**被打分过**的命中（窗口外的条连分都没有，不受它影响）。
    # 缺省 None（不设阈值）与 retrieval_min_score 同一句话：标定之前给一个数字
    # 是假的安全感。差别是这一项的**标定难度低得多**——教学替身的重排分落在
    # [0, 1]，因此一个阈值在这里有绝对含义（不像 BM25 那样依赖语料）。
    retrieval_rerank_min_score: float | None = None
    # 生成与提示词（M6-D8）：模型被怎么问、算不算数。
    # 这一组与前两组是**同一类**（都不改变任何产物的字节：
    # 不改 chunk_id、不改向量、不改清单版本号），
    # 因此调它们随时生效、**不需要**重建索引；它也遵守 day067 定下的那条纪律：
    # 新能力**默认不生效**（下面两处开关的缺省值都是"关"）。
    # 但它比前两组"重"一档，差别必须写出来：
    # retrieval_hybrid_* / retrieval_rerank_* 只改变**名单**（次序、条数），
    # 而 retrieval_prompt_version 换一版会**改变答案文本本身**——同一份索引、
    # 同一个问题，v1 与 v2 交给用户的是两段不同的话。因此换版本号是一次
    # 需要留痕的实验（报告按它分组，见 generation.PROMPT_CHANGELOG）。
    # 提示词版本：v1（day066 首版）/ v2（默认，四段式 + 固定拒答句式）。
    # 历史版本永不删（与 agent.prompts 的约定一致），清单外的取值当场报
    # GenerationError；要注入自己的模板请用 prompt=...，版本号记为 custom。
    retrieval_prompt_version: str = "v2"
    # 生成温度。缺省 0.0 与 day066 的取值一致：RAG 问答是**有依据的复述**，
    # 温度越高模型越会改写片段里的事实（而引用编号仍然对得上，
    # 于是"改了事实"这件事在报告里看不出来）。上限 2.0，越界当场报错。
    retrieval_answer_temperature: float = 0.0
    # 一次回答的长度上限（token）。缺省 1024 与 BaseLLM.chat 的缺省一致：
    # 给"先结论后依据"留够空间；调得太小会得到**空回复**（模型没有空间说话），
    # 而空回复与"资料里没有相关内容"是两件必须分开读的事（见 generation）。
    retrieval_answer_max_tokens: int = 1024
    # 是否把"答案里一条合法引用都没有"当作不可交付。缺省 False：
    # 打开它之后，模型只是忘了写 [n] 就会被整条丢掉（答案取兜底答复），
    # 因此它是一道**闸门**而不是一个阈值——先让默认可核对率跑一段时间，
    # 有了数据再决定要不要把它拧上（day071 的 RAG 评估会给出那个数字）。
    retrieval_require_citation: bool = False

    # ------------------------------------------------------------------ #
    # RAG 评估（day071 / M6-D9）：评测集、判据、基线与门禁
    # ------------------------------------------------------------------ #
    # 这一组与上面那些"怎么问"的旋钮**不是同一类**：它们不改变任何产物的字节，
    # 也不改变"怎么问"——它们只决定**怎么评**（评哪一批用例、判多严、
    # 基线存到哪、门禁的容忍度是多少）。因此它们调了立刻生效，
    # 既不重建索引、也不影响线上任何一次回答。
    # 评测集与语料：与 day029 的 RAG 评估共用**同一份文件**（字段名一致），
    # 因此"检索评估"与"端到端评估"读的是同一批金标准——两份金标准一定会分家，
    # 而分家的表现是"同一个系统在两张报告里的召回率不一样"。
    rag_eval_cases_path: str = "data/eval/rag_eval.jsonl"
    rag_eval_corpus_path: str = "data/eval/rag_corpus.jsonl"
    # 质量基线文件（随仓库提交：它就是这次迭代的质量体检底片）。
    # 没有它与"基线全是零"是两回事，因此 RagBaseline.load 在文件不存在时
    # **报错**而不是返回空基线（见 baseline 模块的说明）。
    rag_eval_baseline_path: str = "data/eval/rag_baseline.json"
    # 完整报告落盘位置（含逐条证据与坏例表）。基线与报告分开存：
    # 基线是"要进 git 的那 11 个数字"，报告是"这次排查的全套证据"。
    rag_eval_report_path: str = "outputs/rag_eval/report.json"
    # 四个检索指标共用的 k（"前几条算命中"）。缺省 3 与评测集的规模匹配：
    # 语料只有 6 条，k=3 已能分辨"抓到了 / 没抓到"；调大它会让指标变宽松，
    # 而宽松的判据在报告里读起来"更健康"（这正是要把它显式写出来的原因）。
    rag_eval_top_k: int = 3
    # 覆盖率下限：低于它算"低覆盖"坏例（给了不少片段，用上的很少）。
    rag_eval_min_coverage: float = 0.5
    # 是否要求答案接地（有可核对的引用）。缺省 True 与"先量一次默认可核对率"
    # 的用法并存：把它关掉跑一次，得到的就是 retrieval_require_citation
    # 缺省关闭时的那份数字。
    rag_eval_require_grounded: bool = True
    # 门禁的三门参数：死区（多大的变化算噪声）、最小样本量（几条用例以下
    # 拒绝下结论）、以及四个关键指标的容忍度（单位是百分点，**差值**口径）。
    # 质量侧用差值而不是比值，因为 hallucination_rate 的健康基线就是 0——
    # 比值在 0 基数下没有定义，差值照样可判（0 → 0.05 就是 5 个百分点）。
    rag_eval_dead_band: float = 0.02
    rag_eval_min_samples: int = 3
    rag_eval_recall_tolerance: float = 0.05
    rag_eval_grounded_tolerance: float = 0.05
    rag_eval_coverage_tolerance: float = 0.08
    rag_eval_hallucination_tolerance: float = 0.05

    # ------------------------------------------------------------------ #
    # RAG 生产化运维（day072 / M6-D10）：语料水位、调度节奏、编排端口
    # ------------------------------------------------------------------ #
    # 这一组与前两组的关系是"上游/下游"，而不是并列：
    #   `rag_eval_*`（day071）决定**怎么评**，本组决定**谁来定期跑、跑完通知谁**。
    # 它们同样不改变任何产物的字节（不改 chunk_id、不改向量、不改清单版本号），
    # 只改变运维节奏，因此调了立刻生效。
    # 但本组里有两项**会改变产物**，必须单独点出来：
    #   `rag_ops_source_dir`  换了它等于换了一批入库文档（下一趟同步就会体现）
    #   `rag_ops_churn_full_rebuild` 换了它会让同一批语料走"全量重建"（版本号照变）
    # 语料根目录（同步的输入）。缺省指向 data/knowledge：本仓库自带的那份语料
    # 就在那里，因此开箱即可跑通一次真实的同步。换成真实语料时请显式配置。
    rag_ops_source_dir: str = "data/knowledge"
    # 同步账本（水位 + 尝试史）。**这份文件刻意不进 git**（见 .gitignore）：
    # 账本进了 git，一次全新的部署会以为"我已经同步过了"，于是第一次运行
    # 只做增量——而增量在空库上等于什么都没做。没有账本才是正确的首次状态。
    rag_ops_ledger_path: str = "data/ops/sync_ledger.json"
    # 运行报告落盘位置（每次运行一份，含四块内容与全部 notes）。
    # 与账本分开：账本是**状态**（必须能被下一次运行读到），报告是**证据**（给人看）。
    rag_ops_report_path: str = "outputs/rag_ops/ops_report.json"
    # 编排文件路径（由 deploy.render_compose 渲染、随仓库提交、被测试逐字节守着）。
    # 与既有的 docker-compose.yml（MCP Server，day022）并存而不是覆盖：
    # 两套编排的端口与角色都不同，合成一份会让"这个服务属于哪一套"变成读全文的问题。
    rag_ops_compose_path: str = "docker-compose.rag.yml"
    # 调度节奏：间隔 1440 分钟 = 每天一次（本课程的自动运行就是"每天凌晨一趟"）。
    # 抖动 300 秒把多实例的计划点错开，避开整点争抢；它必须小于一个间隔
    # （否则"下一次"这个答案会失去意义，见 SchedulePolicy 的校验）。
    rag_ops_interval_minutes: int = 1440
    rag_ops_jitter_seconds: int = 300
    # 滞后多久算"错过了一个窗口"：720 分钟 = 半天。超过它只会**多一条理由**，
    # 不会吞掉本次运行——"迟到就不再跑"是运维里最昂贵的一种自作聪明。
    rag_ops_max_lag_minutes: int = 720
    # 失败退避：60 秒起步、指数增长、3600 秒封顶。封顶的意义是"永远会重试"，
    # 而指数增长的意义是"坏掉的东西不要每 60 秒撞一次提供方"。
    rag_ops_backoff_base_seconds: int = 60
    rag_ops_backoff_max_seconds: int = 3600
    # 语料新鲜度阈值（分钟）：2160 = 36 小时 = "一天一次"的 1.5 倍。
    # 取 1.5 倍而不是 1 倍：一次迟到 2 小时的正常调度不该被报成"语料过期"，
    # 而"整整一天没同步"必须被报出来。
    rag_ops_freshness_minutes: int = 2160
    # 变更比例超过它就从"增量"换成"整库重建"。与 day065 的
    # `indexing_full_rebuild_threshold` 是**同一个口径的两个使用点**：
    # 那边按块算，这边按文件算（文件级先判一次，能省下整轮解析与分块）。
    rag_ops_churn_full_rebuild: float = 0.5
    # 编排端口：应用 8080（避开既有 docker-compose.yml 的 8000）、
    # 向量库 8000（官方 Chroma 镜像的默认端口，见 deploy.CHROMA_IMAGE）。
    rag_ops_api_port: int = 8080
    rag_ops_store_port: int = 8000
    # 应用镜像 tag。**不许用 latest**（deploy 的 R4 会当场拒绝）：
    # 今天与明天拉到的是两个镜像，而配置看起来一模一样。
    rag_ops_image_tag: str = "v0.1.0"
