...
day040 起补齐视觉能力：
  - ``supports_vision`` / ``chat_vision``：图像请求只路由给支持视觉的候选，
    纯文本模型（不声明 supports_vision）在视觉链里被直接跳过；
  - 降级语义与 chat 一致：视觉首选失败时沿降级链找下一个视觉模型。
"""
...
class ModelRouter(BaseLLM):
    ...
    def last_used_llm(self) -> BaseLLM | None:
        ...
        存在的理由：计费与用量统计（day043 的 ``CostTracker.record_from_llm``）
        读的是 ``llm.usage_log``，而 ``ModelRouter`` 本身不产生 token、也不
        持有用量——它是包装器。若上层把路由器直接交给计费器，就会在
        "包装器没有 usage_log"处断链。把叶子暴露出来，让计费落到真正
        干活的那个客户端上，用量账目才能与决策日志（``decision_log``）对齐：
        决策日志说"选了谁"，本属性回答"谁真的花了钱"。
        """
    ...
    def supports_vision(self) -> bool:
        ...
        视觉不是每个模型都有的能力：路由器的``chat_vision`` 只把图像请求
        交给声明支持视觉的候选，纯文本模型直接跳过。``supports_vision``
        就是路由层判断"有没有视觉候选可用"的依据。
        """
    ...
    def chat_vision(
        ...
        """视觉请求路由（day040）：只把图像请求发给支持视觉的候选.

        与 chat 对称的降级语义：先按最近一条 user 消息做一次路由决策，
        然后在降级链里**只挑选**支持视觉的模型尝试；文本模型被跳过而非
        报错——把"该模型没有视觉能力"当成路由层可规避的事实，而不是
        一次失败。全部视觉候选都失败时，按 chat 的惯例汇总错误抛出。
        """
        decision, chain = self.route(self._last_user_task(messages))
        errors: list[str] = []
        for spec in chain:
