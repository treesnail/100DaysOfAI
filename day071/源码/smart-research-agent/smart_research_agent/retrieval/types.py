...
#: day069 第三次改写，依据是同一条纪律（**兑现之后剩下的那条才算边界**）：
#: 引用核对与接地判定在 ``generation.py`` 里落地了（``GroundingReport`` 的三组
#: 编号 + 覆盖率），因此"不做引用溯源校验"这类说法在本层不再成立；
#: 换上来的两条是兑现之后**仍然成立**的边界：核对只在编号层面、
#: 生成器只做推理侧。剩下的每一条都指着一个别的模块或另一天。
#: （注：``api.routes`` 的 docstring 里还留着"不做引用溯源校验（day069）"一句，
#: 那属于端点那一天的活，本模块不动它——但这句话现在与代码不符了。）
RETRIEVAL_LIMITATIONS: tuple[str, ...] = (
    "接地校验只做**引用编号**层面的核对：答案里的 [n] 与那次提示词的编号对账"
    "（有效 / 幻觉 / 未引用 + 覆盖率），**不做语义层面的忠实性判断**——"
    "'这句话有没有歪曲片段'需要另一套方法（day069 的 check 不回答它，"
    "所以它叫 grounded 而不是 correct）",
    "生成器只做**推理侧**：它按版本挑一份模板、把片段喂进去、核对回来的引用；"
    "提示词的**训练与微调**不在本课内（day069 只给推理接口 RAGGenerator）",
    "拒答检测靠固定句式（DECLINE_MARKERS）：v2 的约束里写死了那三个句式，"
    "模型换个说法（或换一版不要求该句式的模板）就检测不到——"
    "这条边界的代价是'宁可漏检，不可误判'",
...
#: day069 补进来的那条是**别人那一天的活**：答案质量评估（有用性 / 准确性打分）
#: 属于 day071，本层只给"可核对"这一个更窄的问题一个答案。
RETRIEVAL_OUT_OF_SCOPE: tuple[str, ...] = (
    "答案质量评估（有用性 / 准确性打分）：属于 day071，不在本层"
    "——本层回答的是'这段话有没有可核对的依据'，不是'这段话答得好不好'",
