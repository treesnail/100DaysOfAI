...
def parse_faithfulness(text: str) -> FaithfulnessResult:
    """公开的忠实度解析入口（day071 起；**只增不改**）.

    day071 的端到端 RAG 评估（``rag_debug``）也要评忠实度，而它用的是
    同一份提示词与同一套防御式解析。把 ``_parse_faithfulness`` 公开出来
    而不是在新模块里重写一遍，理由与 day069 把核对收进一处完全相同：
    **同一件事只有一份实现**——两份解析一定会分家，而分家的表现是
    "同一段评审输出在旧报告里是 1.0、在新报告里是报错"。

    这是**只增不改**：``_parse_faithfulness`` 仍然在（它被本函数与
    ``answer_faithfulness`` 共用），既有调用方的行为一个字节都没变。
    """
    return _parse_faithfulness(text)
