...
为什么**只有**这几条？        → dropped_* 三个数字 + empty_reason（唯一原因）
...
RETRIEVAL_LIMITATIONS: tuple[str, ...] = (
    "只做单路向量检索：不实现 BM25、不做多路融合与去重（day067 的混合检索另建一层）",
    "不带重排序：命中的顺序完全由向量度量决定（day068 加交叉编码器）",
    "阈值在本层落刀而不是交给库：因为'切掉了几条'这个数字必须能被报告（见 retriever）",
...
class RetrievalQuery:
    ...
    ``extra`` 是 day067 融合参数的预留槽（``{"alpha": 0.3, "k_rrf": 60}`` 之类）。
    本课**不使用它**，但形状先留出来：融合层要能挂参数而不改这一层的签名。
...
class RetrievalResult:
    ...
    def __post_init__(self) -> None:
        ...
        for name in ("fetch_k", "candidates", "dropped_below_threshold",
                     "dropped_by_diversity", "dropped_by_top_k"):
    ...
    def explain(self) -> list[str]:
        ...
        3) 为什么条数少于 top_k          → 第三行（三个 dropped_* 数字）
