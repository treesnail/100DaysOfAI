"""混合检索：**两路一起答，并说清每一条是哪一路顶上来的**（M6-D6）.

day066 交了单路向量检索器（一行流水线九步，三道减法各自留数字）。
今天这一层不重写那条流水线，只回答一个新问题：
...
13. 诊断与装配    empty_reason / channel_candidates / fusion 摘要 / notes
```
...
class HybridRetriever:
    ...
    构造参数分两组，与 day066 的 ``Retriever`` 同一风格：
    ...
    ``None`` 在融合类参数上有明确含义：**"没指定，去读 settings 或去问向量路"**
    （``retrieval_hybrid_strategy`` / ``retrieval_hybrid_alpha`` /
    ``retrieval_hybrid_rrf_k``）。于是"项目默认值"只有一处定义，
    端点与演示脚本不必把三个数字抄两遍。

    **它刻意不继承 ``Retriever``**：那条流水线（九步）与这条（十三步）在
    "什么时候落阈值、什么时候裁多样性"上顺序**不同**——混合模式必须把
    多样性放在融合之后（否则两路各自裁一遍，被某一路裁掉的记录就没有机会
    从另一路被救回来）。继承会强迫这两套顺序共用一套方法名，
    而它们的语义差别恰恰是本课要讲的东西。
    ...
    def describe(self) -> dict[str, Any]:
        ...
        return {
            ...
            "max_per_doc": self._max_per_doc if self._max_per_doc is not None
    ...
    def retrieve(self, query: str | RetrievalQuery) -> RetrievalResult:
        ...
        # 第 12 步：截断（次序是融合的权威次序，因此只重编号、不重排）。
    ...
    def explain(self, result: RetrievalResult) -> list[str]:
        """在 ``result.explain()`` 之上补**混合模式特有**的四行诊断.

        ``result.explain()`` 的四问（哪一版索引 / 什么过滤 / 为什么少 /
        为什么空）在混合模式下全部照旧；这里补的是那条通道回答不了的四件事：
    ...
    def _resolve_fusion(
        ...
        unknown = sorted(set(extra) - set(FUSION_OVERRIDE_KEYS))
        if unknown:
            raise FusionError(
                f"RetrievalQuery.extra 里有本层不认识的键 {unknown}："
                f"混合检索只认 {'、'.join(FUSION_OVERRIDE_KEYS)}"
                "（extra 是 day066 为融合参数预留的槽）。"
