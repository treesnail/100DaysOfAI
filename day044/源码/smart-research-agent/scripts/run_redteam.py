...
day032 起支持门禁阈值：--min-block-rate，整体拦截率低于阈值时
报告追加红色告警段并以退出码 1 结束（供 CI/流水线判定）。
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--min-block-rate", type=float, default=0.0, help="整体拦截率门禁阈值"
    )
    ...
    alerts = []
    if args.min_block_rate > 0.0:
        alerts = check_thresholds(
            {"block_rate": report.block_rate}, {"block_rate": args.min_block_rate}
        )
    if alerts:
        markdown += render_alerts(alerts)
        for alert in alerts:
