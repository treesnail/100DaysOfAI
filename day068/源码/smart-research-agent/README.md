...
M6 - 检索增强（**进行到 day068 / M6-D7**）：day066 单路检索 → day067 混合检索（BM25 + 融合）
→ day068 重排序（交叉编码器只对前 N 条打分）。重排是**缺省关闭**的新能力
（`retrieval_rerank_enabled=False`），关着时 day066/day067 的结果逐位不变。
