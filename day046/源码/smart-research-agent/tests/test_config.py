...
class TestDefaultValues:
    ...
    def test_integration_defaults(self, default_settings: Settings):
        """一体化流水线与性能基线默认值（day046）：六层全开，容忍度保守."""
        assert default_settings.pipeline_cache_enabled is True
        assert default_settings.pipeline_block_on_injection is True
        assert default_settings.perf_baseline_path == "data/eval/perf_baseline.json"
        assert default_settings.perf_latency_tolerance == 1.5
        assert default_settings.perf_token_tolerance == 1.2
        assert default_settings.perf_cost_tolerance == 1.2
